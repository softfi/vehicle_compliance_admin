...
<div class="page-body-wrapper">
    ...
    <div class="page-body">
        <div class="container-fluid">
            ...
            <div class="card">
                <div class="card-body">
                    <form method="POST" action="<?= base_url('admin/attendance/update/' . $attendance->id); ?>">
                        <?= csrf_field(); ?>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" id="staff_id" name="staff_id" required>
                                        ...
                                        <?php foreach ($staff as $s): ?>
                                            <option data-doj="<?= $s->doj ?? ''; ?>" data-resign-date="<?= $s->resign_date ?? ''; ?>" value="<?= $s->id; ?>" <?= $attendance->staff_id == $s->id ? 'selected' : ''; ?>>
                                                <?= $s->name; ?> (<?= $s->staff_code; ?>)
                                            </option>
                                        <?php endforeach; ?>
                                    </select>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select class="form-control" id="status" name="status" required onchange="toggleLeaveType()">
                                        ...
                                        <option value="Sick-leave" <?= $attendance->status == 'Sick-leave' ? 'selected' : ''; ?>>Sick-leave</option>
                            ...
                            <div class="col-md-6" id="leave_type_div" style="display: none;">
                                <div class="form-group">
                                    ...
                                    <select class="form-control" id="leave_type" name="leave_type">
                                        ...
                                        <option value="Annual" <?= $attendance->leave_type == 'Annual' ? 'selected' : ''; ?>>Annual Leave</option>
                                        <option value="Casual" <?= $attendance->leave_type == 'Casual' ? 'selected' : ''; ?>>Casual Leave</option>
                                        <option value="Sick" <?= $attendance->leave_type == 'Sick' ? 'selected' : ''; ?>>Sick Leave</option>
                                        <option value="Unpaid" <?= $attendance->leave_type == 'Unpaid' ? 'selected' : ''; ?>>Unpaid Leave</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" id="notes" name="notes" rows="4" placeholder="Enter any additional notes..." maxlength="500"><?= $attendance->notes; ?></textarea>
    ...
    for (let i = 0; i < options.length; i++) {
        ...
        if (show) {
            ...
            if (currentSelected === option.value) {
                // For edit page, we might want to keep it if it's the already saved value
                // but usually it's better to warn or block.
            }
        }
    }
}
