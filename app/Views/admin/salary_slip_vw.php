...
<head>
    ...
    <style>
        @page { margin: 8mm; }
        body { font-family: 'DejaVu Sans', sans-serif; background-color: white; padding: 0; color: #333; margin: 0; font-size: 11px; }
        .page-container { width: 100%; margin: 0 auto; padding: 0; }
        .salary-card, .trip-card { background: white; padding: 12px; border: 1px solid #eee; margin-bottom: 15px; width: 100%; box-sizing: border-box; page-break-inside: avoid; }
        ...
        .header h1 { margin: 0; color: #2c3e50; text-transform: uppercase; letter-spacing: 1.5px; font-size: 20px; }
        .header p  { margin: 5px 0; color: #7f8c8d; font-weight: bold; }

        .info-table, .data-table, .trip-table, .diesel-fill-table { width: 100%; border-collapse: collapse; margin-bottom: 15px; table-layout: fixed; }
        .info-table td { padding: 8px; border: 1px solid #eee; font-size: 14px; }
        .label { font-weight: bold; background-color: #f9f9f9; width: 25%; }

        .data-table th, .trip-table th, .diesel-fill-table th { background-color: #2c3e50; color: white; padding: 8px; text-align: left; font-size: 12px; }
        .data-table td, .trip-table td, .diesel-fill-table td { padding: 8px; border: 1px solid #ddd; font-size: 11px; word-wrap: break-word; overflow-wrap: break-word; }

        .total-row { background-color: #ecf0f1; font-weight: bold; }

        .summary-box { background: #2c3e50; color: white; border-radius: 5px; margin-top: 10px; width: 100%; }
        .summary-box td { padding: 12px 15px; vertical-align: middle; }
        .summary-box h2 { margin: 0; font-size: 15px; line-height: 1.4; color: white; }
        .balance-highlight { color: #f1c40f; font-size: 17px; font-weight: bold; }

        .footer { margin-top: 20px; text-align: center; font-size: 11px; color: #999; border-top: 1px dashed #ccc; padding-top: 10px; }
        .signature-section { margin-top: 30px; font-weight: bold; color: #555; }
        ...
        .trip-count-badge { color: #2c3e50; font-weight: bold; font-size: 11px; }
        .vehicle-tag { color: #2c3e50; font-weight: bold; border-bottom: 1px solid #2c3e50; padding: 0 2px; margin: 0 2px; font-size: 10px; }
        ...
        .veh-title-bar { background: #2c3e50; color: white; padding: 10px 16px; }
        .veh-title-bar table { width: 100%; }
        ...
        .oc-header-bar { background: #1a252f; width: 100%; table-layout: fixed; }
        .oc-box { padding: 8px 5px; text-align: center; border-right: 1px solid rgba(255,255,255,.15); }
        .oc-box:last-child { border-right: none; }
        .oc-label { font-size: 9px; color: rgba(255,255,255,.65); text-transform: uppercase; letter-spacing: .5px; margin-bottom: 2px; }
        .oc-value { font-size: 16px; font-weight: bold; }
        ...
        .veh-summary-bar { background: #eef6ff; border-top: 2px solid #2c3e50; }
        .vsb-box { padding: 10px; text-align: center; border-right: 1px solid #c8dcf0; }
...
<!-- Print button removed for PDF download -->
...
<div class="salary-card">
        ...
        <tr>
            <td style="width:50%; vertical-align: top; padding-right: 10px;">
                <table class="data-table" style="width:100%;">
                    ...
                    <tr><td>Basic Salary</td>                  <td class="amount-cell">Rs. <?= number_format($basic_salary, 2) ?></td></tr>
                    <tr><td>Trip Expenses</td>                 <td class="amount-cell">Rs. <?= number_format($trip_expenses, 2) ?></td></tr>
                    <tr><td>Bonus / Incentives</td>            <td class="amount-cell">Rs. <?= number_format($bonus, 2) ?></td></tr>
                    <tr><td>Opening Balance (Credit)</td>      <td class="amount-cell">Rs. <?= $opening > 0 ? number_format($opening, 2) : '0.00' ?></td></tr>
                    <tr class="total-row">
                        <td>Total Earnings</td>
                        <td class="amount-cell">Rs. <?= number_format($basic_salary + $trip_expenses + $bonus + ($opening > 0 ? $opening : 0), 2) ?></td>
                    </tr>
                </table>
            </td>
            <td style="width:50%; vertical-align: top; padding-left: 10px;">
                <table class="data-table" style="width:100%;">
                    ...
                    <tr><td>Salary Advance</td>                <td class="amount-cell">Rs. <?= number_format($salary_advance, 2) ?></td></tr>
                    <tr><td>Trip Advance</td>                  <td class="amount-cell">Rs. <?= number_format($trip_advance, 2) ?></td></tr>
                    <tr><td>Adjustments / Penalties</td>       <td class="amount-cell">Rs. <?= number_format($adjustment, 2) ?></td></tr>
                    <tr><td>HSD / Diesel (Extra Usage)</td>    <td class="amount-cell">Rs. <?= number_format(abs($hsd_amount), 2) ?></td></tr>
                    <tr><td>Opening Balance (Debit)</td>       <td class="amount-cell">Rs. <?= $opening < 0 ? number_format(abs($opening), 2) : '0.00' ?></td></tr>
                    <tr class="total-row">
                        <td>Total Deductions</td>
                        <td class="amount-cell">Rs. <?= number_format($total_advance + $adjustment + abs($hsd_amount) + ($opening < 0 ? abs($opening) : 0), 2) ?></td>
                    </tr>
                </table>
            </td>
        </tr>
    ...
    <table class="summary-box" style="width:100%; border-spacing:0;">
        <tr>
            <td>
                <h2>Net Salary Payable: Rs.  <?= number_format($net_salary, 2) ?></h2>
                <div style="font-size:11px; opacity:0.8;">Paid Amount: Rs.  <?= number_format($total_paid, 2) ?></div>
            </td>
            <td style="text-align:right;">
                <span style="font-size:11px; opacity:0.8;">REMAINING BALANCE: </span>
                <span class="balance-highlight">Rs.  <?= number_format($balance, 2) ?></span>
            </td>
        </tr>
    </table>
...
<div class="trip-card">
    ...
    <div class="vehicle-block">
        ...
        <div class="veh-title-bar">
            <table style="width:100%; border-collapse: collapse;">
                <tr>
                    <td class="veh-name" style="color:white; font-weight:bold;">🚛 &nbsp;<?= htmlspecialchars($vd['vehicle_no']) ?></td>
                    <td class="veh-period" style="color:white; text-align:right; font-size:12px;">
        ...
        <table class="oc-header-bar" style="width:100%; border-collapse: collapse;">
            <tr>
                <td class="oc-box" style="width:20%;">
        ...
        <table class="veh-summary-bar" style="width:100%; border-collapse: collapse;">
            <tr>
                <td class="vsb-box" style="width:20%;">
    ...
    <table style="width:100%; border-collapse: collapse; background:#2c3e50; color:white; border-radius:5px; margin-bottom:30px;">
        <tr>
            <td style="width:50%; padding:12px 20px; text-align:center;">
    ...
    <table class="trip-table">
        <thead>
            <tr>
                ...
                <th style="width: 25%;">DO No</th>
                <th style="width: 15%;">Vehicle(s)</th>
                <th style="width: 35%;">Route</th>
                <th style="width: 10%; text-align:center;">Trips</th>
                <th style="width: 10%; text-align:right;">Diesel (L)</th>
            </tr>
        </thead>
        <tbody>
        <?php
    ...
    ?>
    <table style="width:100%; border-collapse: collapse; border:1px solid #2c3e50; background:#eef6ff; border-radius:5px; margin-top:5px;">
        <tr>
    ...
    <div class="signature-section">
        <table style="width:100%; border-collapse: collapse;">
            <tr>
                <td style="width:50%; text-align:left;">Verified By: _________________</td>
                <td style="width:50%; text-align:right;">Driver's Signature: _________________</td>
            </tr>
        </table>
    </div>
