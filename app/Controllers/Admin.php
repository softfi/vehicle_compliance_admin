...
class Admin extends BaseController
{
    ...
    function vehicle()
    {
        ...
        $data['vehicle_types'] = $vehicleTypeModel->getActiveTypes();
    ...
    function Insertvehicle()
    {
        if ($this->session->get('user_id')) {
            ...
            if ($this->validate($rules)) {
                ...
                session()->setFlashdata('msg', 'Vehicle Added Successfully');
                return redirect()->to('Admin/Vehicle');
            } else {
                return redirect()->back()->withInput()->with('error', 'Validation Failed. Please check all fields.')->with('validation', $this->validator);
            }
        } else {
            return redirect()->to('Admin/');
        }
    ...
    public function upload()
    {
        ...
        if ($file->isValid() && !$file->hasMoved()) {
            ...
            foreach ($data as $index => $row) {
                // Skip the header row and completely empty rows
                if ($index === 0 || empty(trim($row[0])))
                    continue;

                $vehicle_no = $row[0];
                $CountVehicle = $this->db->query("SELECT * FROM vehicle  where vehicle_no='$vehicle_no'")->getResult();

                $vehicle_type_name = isset($row[1]) ? trim($row[1]) : '';
                $vehicle_type = 1; // Default to 1 (Truck) if not found
                if (!empty($vehicle_type_name)) {
                    $typeResult = $this->db->table('vehicle_types')->like('type_name', $vehicle_type_name)->get()->getRow();
                    if ($typeResult) {
                        $vehicle_type = $typeResult->id;
                    }
                }

                $fitness_expairy_date = isset($row[4]) ? $row[4] : null;
                $tax_exp_date = isset($row[6]) ? $row[6] : null;
                $ins_date = isset($row[9]) ? $row[9] : null;
                $permit_ex = isset($row[11]) ? $row[11] : null;
                $npermit_ex = isset($row[13]) ? $row[13] : null;
                $amc_ex = isset($row[27]) ? $row[27] : null;
                $pucc_ex = isset($row[28]) ? $row[28] : '';
                $im3_ex = isset($row[30]) ? $row[30] : null;
                $khanija_ex = isset($row[32]) ? $row[32] : null;

                $vehicleData = [
                    'vehicle_no' => isset($row[0]) ? $row[0] : '',
                    'vehicle_type' => $vehicle_type,
                    'chassis_no' => isset($row[2]) ? $row[2] : '',
                    'engine_no' => isset($row[3]) ? $row[3] : '',
                    'fitness_exp_date' => date('Y-m-d', strtotime($fitness_expairy_date)),
                    'fitness_amount' => isset($row[5]) ? $row[5] : 0.00,
                    'tax_exp_date' => date('Y-m-d', strtotime($tax_exp_date)),
                    'road_tax_amount' => isset($row[7]) ? $row[7] : 0.00,
                    'ins_company' => isset($row[8]) ? $row[8] : '',
                    'ins_exp_date' => date('Y-m-d', strtotime($ins_date)),
                    'insurance_amount' => isset($row[10]) ? $row[10] : 0.00,
                    'permit_exp_date' => date('Y-m-d', strtotime($permit_ex)),
                    'permit_amount' => isset($row[12]) ? $row[12] : 0.00,
                    'npermit_exp_date' => date('Y-m-d', strtotime($npermit_ex)),
                    'npermit_amount' => isset($row[14]) ? $row[14] : 0.00,
                    'finance' => isset($row[15]) ? $row[15] : '',
                    'deduct_amount' => isset($row[16]) ? $row[16] : 0.00,
                    'emi_account' => isset($row[17]) ? $row[17] : '',
                    'horse_make' => isset($row[18]) ? $row[18] : '',
                    'horse_model' => isset($row[19]) ? $row[19] : '',
                    'horse_rate' => isset($row[20]) ? $row[20] : 0.00,
                    'dala_rate' => isset($row[21]) ? $row[21] : 0.00,
                    'dala_make' => isset($row[22]) ? $row[22] : '',
                    'rto_expenses' => isset($row[23]) ? $row[23] : 0.00,
                    'amc' => isset($row[24]) ? $row[24] : '',
                    'amc_amount' => isset($row[25]) ? $row[25] : 0.00,
                    'amc_frequancy' => isset($row[26]) ? $row[26] : '',
                    'amc_expary' => date('Y-m-d', strtotime($amc_ex)),
                    'pucc' => date('Y-m-d', strtotime($pucc_ex)),
                    'pucc_amount' => isset($row[29]) ? $row[29] : '',
                    'i3ms_expary' => date('Y-m-d', strtotime($im3_ex)),
                    'i3ms_recharge' => isset($row[31]) ? $row[31] : '',
                    'khanij_expiri' => date('Y-m-d', strtotime($khanija_ex)),
                    'khanij_amount' => isset($row[33]) ? $row[33] : null,
                    'remark' => isset($row[34]) ? $row[34] : '',
    ...
    public function download_vehicle_sample_excel()
    {
        ...
        $fileName = 'Vehicle_Sample_Excel.xlsx';
    ...
    function edit_vehicle()
    {
        ...
        $vehicleTypeModel = new \App\Models\VehicleTypeModel();
        $vehicle_types = $vehicleTypeModel->getActiveTypes();
                ...
                <div class="form-group uk-margin-bottom">
                    ...
                    <select class="form-control" name="vehicle_type" id="edit_vehicle_type" required>
                        ...
                        <?php foreach ($vehicle_types as $type): ?>
                            <option value="<?= $type->id ?>" <?= ($vec->vehicle_type == $type->id) ? 'selected' : '' ?>>
                ...
                <div class="uk-child-width-1-2@m uk-grid-small" uk-grid>
                    <div class="form-group">
                        ...
                        <input class="form-control" type="text" name="vehicle_no" id="vehicle_no"
                            value="<?= $vec->vehicle_no ?>">
                        <?php if (isset($validation)) { ?><span
                                class="text-danger"><?= $error = $validation->getError('vehicle_no'); ?></span><?php } ?>
                    </div>
                    <div class="form-group">
                        <label for="chassis_no">Chassis Number</label>
                        <input class="form-control" type="text" name="chassis_no" id="chassis_no"
                            value="<?= $vec->chassis_no ?>">
                        <?php if (isset($validation)) { ?><span
                                class="text-danger"><?= $error = $validation->getError('chassis_no'); ?></span><?php } ?>
                    </div>
                    <div class="form-group">
                        ...
                        <?php if (isset($validation)) { ?><span
                                class="text-danger"><?= $error = $validation->getError('engine_no'); ?></span><?php } ?>
                    </div>
                    <div class="form-group">
                        <label for="fitness_exp_date">Fitness Expiry Date</label>
                        <input class="form-control" type="date" name="fitness_exp_date" id="fitness_exp_date"
                            value="<?= $vec->fitness_exp_date ?>">
                        <?php if (isset($validation)) { ?><span
                                class="text-danger"><?= $error = $validation->getError('fitness_exp_date'); ?></span><?php } ?>
                    </div>
                    <div class="form-group">
                        <label for="fitness_amount">Fitness Amount</label>
                        <input class="form-control" type="text" name="fitness_amount" id="fitness_amount"
                            value="<?= $vec->fitness_amount ?>">
                        <?php if (isset($validation)) { ?><span
                                class="text-danger"><?= $error = $validation->getError('fitness_amount'); ?></span><?php } ?>
                    </div>
                    <div class="form-group">
                        <label for="tax_exp_date">Road Tax Expiry Date</label>
                        <input class="form-control" type="date" name="tax_exp_date" id="tax_exp_date"
                            value="<?= $vec->tax_exp_date ?>">
                        <?php if (isset($validation)) { ?><span
                                class="text-danger"><?= $error = $validation->getError('tax_exp_date'); ?></span><?php } ?>
                    </div>
                    <div class="form-group">
                        <label for="road_tax_amount">Road Tax Amount</label>
                        <input class="form-control" type="text" name="road_tax_amount" id="road_tax_amount"
                            value="<?= $vec->road_tax_amount ?>">
                        <?php if (isset($validation)) { ?><span
                                class="text-danger"><?= $error = $validation->getError('road_tax_amount'); ?></span><?php } ?>
                    </div>
                    <div class="form-group">
                        <label for="ins_company">Insurance Company</label>
                        <input class="form-control" type="text" name="ins_company" id="ins_company"
                            value="<?= $vec->ins_company ?>">
                        <?php if (isset($validation)) { ?><span
                                class="text-danger"><?= $error = $validation->getError('ins_company'); ?></span><?php } ?>
                    </div>
                    <div class="form-group">
                        <label for="ins_exp_date">Insurance Expiry Date</label>
                        <input class="form-control" type="date" name="ins_exp_date" id="ins_exp_date"
                            value="<?= $vec->ins_exp_date ?>">
                        <?php if (isset($validation)) { ?><span
                                class="text-danger"><?= $error = $validation->getError('ins_exp_date'); ?></span><?php } ?>
                    </div>
                    <div class="form-group">
                        <label for="insurance_amount">Insurance Amount</label>
                        <input class="form-control" type="text" name="insurance_amount" id="insurance_amount"
                            value="<?= $vec->insurance_amount ?>">
                        <?php if (isset($validation)) { ?><span
                                class="text-danger"><?= $error = $validation->getError('insurance_amount'); ?></span><?php } ?>
                    </div>
                    <div class="form-group">
                        <label for="permit_exp_date"> Permit Expiry Date</label>
                        <input class="form-control" type="date" name="permit_exp_date" id="permit_exp_date"
                            value="<?= $vec->permit_exp_date ?>">
                        <?php if (isset($validation)) { ?><span
                                class="text-danger"><?= $error = $validation->getError('permit_exp_date'); ?></span><?php } ?>
                    </div>
                    <div class="form-group">
                        <label for="permit_amount"> Permit Amount</label>
                        <input class="form-control" type="text" name="permit_amount" id="permit_amount"
                            value="<?= $vec->permit_amount ?>">
                        <?php if (isset($validation)) { ?><span
                                class="text-danger"><?= $error = $validation->getError('permit_amount'); ?></span><?php } ?>
                    </div>

                    <div class="form-group">
                        <label for="permit_exp_date">National Permit Expiry Date</label>
                        <input class="form-control" type="date" name="npermit_exp_date" id="npermit_exp_date"
                            value="<?= $vec->npermit_exp_date ?>">
                        <?php if (isset($validation)) { ?><span
                                class="text-danger"><?= $error = $validation->getError('npermit_exp_date'); ?></span><?php } ?>
                    </div>
                    <div class="form-group">
                        <label for="permit_amount">National Permit Amount</label>
                        <input class="form-control" type="text" name="npermit_amount" id="npermit_amount"
                            value="<?= $vec->npermit_amount ?>">
                        <?php if (isset($validation)) { ?><span
                                class="text-danger"><?= $error = $validation->getError('npermit_amount'); ?></span><?php } ?>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <?php if (isset($validation)) { ?><span
                                class="text-danger"><?= $error = $validation->getError('finance'); ?></span><?php } ?>
                    </div>
                    <div class="form-group">
                        <label for="deduct_amount">Deduct Amount</label>
                        <input class="form-control" type="text" name="deduct_amount" id="deduct_amount"
                            value="<?= $vec->deduct_amount ?>">
                        <?php if (isset($validation)) { ?><span
                                class="text-danger"><?= $error = $validation->getError('deduct_amount'); ?></span><?php } ?>
                    </div>
                    <div class="form-group">
                        <label for="emi_account">Account from EMI Deducted</label>
                        <input class="form-control" type="text" name="emi_account" id="emi_account"
                            value="<?= $vec->emi_account ?>">
                        <?php if (isset($validation)) { ?><span
                                class="text-danger"><?= $error = $validation->getError('emi_account'); ?></span><?php } ?>
                    </div>
                    <div class="form-group">
                        <label for="horse_make">Horse Make</label>
                        <input class="form-control" type="text" name="horse_make" id="horse_make"
                            value="<?= $vec->horse_make ?>">
                        <?php if (isset($validation)) { ?><span
                                class="text-danger"><?= $error = $validation->getError('horse_make'); ?></span><?php } ?>
                    </div>
                    <div class="form-group">
                        <label for="horse_model">Horse Model</label>
                        <input class="form-control" type="text" name="horse_model" id="horse_model"
                            value="<?= $vec->horse_model ?>">
                        <?php if (isset($validation)) { ?><span
                                class="text-danger"><?= $error = $validation->getError('horse_model'); ?></span><?php } ?>
                    </div>
                    <div class="form-group">
                        <label for="horse_rate">Horse Rate</label>
                        <input class="form-control" type="text" name="horse_rate" id="horse_rate"
                            value=" <?= $vec->horse_rate ?>">
                        <?php if (isset($validation)) { ?><span
                                class="text-danger"><?= $error = $validation->getError('horse_rate'); ?></span><?php } ?>
                    </div>
                    <div class="form-group">
                        <label for="dala_rate">Dala Rate</label>
                        <input class="form-control" type="text" name="dala_rate" id="dala_rate" value="<?= $vec->dala_rate ?>">
                        <?php if (isset($validation)) { ?><span
                                class="text-danger"><?= $error = $validation->getError('dala_rate'); ?></span><?php } ?>
                    </div>
                    <div class="form-group">
                        <label for="dala_make">Dala Make</label>
                        <input class="form-control" type="text" name="dala_make" id="dala_make" value="<?= $vec->dala_make ?>">
                        <?php if (isset($validation)) { ?><span
                                class="text-danger"><?= $error = $validation->getError('dala_make'); ?></span><?php } ?>
                    </div>
                    <div class="form-group">
                        <label for="rto_expenses">RTO Expenses</label>
                        <input class="form-control" type="text" name="rto_expenses" id="rto_expenses"
                            value="<?= $vec->rto_expenses ?>">
                        <?php if (isset($validation)) { ?><span
                                class="text-danger"><?= $error = $validation->getError('rto_expenses'); ?></span><?php } ?>
                    </div>
                    <div class="form-group">
                        <label for="amc">AMC</label>
                        <input class="form-control" type="text" name="amc" id="amc" value="<?= $vec->amc ?>">
                        <?php if (isset($validation)) { ?><span
                                class="text-danger"><?= $error = $validation->getError('amc'); ?></span><?php } ?>
                    </div>

                    <div class="form-group">
                        <label for="amc">AMC</label>
                        <input class="form-control" type="text" name="amc_frequancy" id="amc"
                            value="<?= $vec->amc_frequancy ?>">
                        <?php if (isset($validation)) { ?><span
                                class="text-danger"><?= $error = $validation->getError('amc_frequancy'); ?></span><?php } ?>
                    </div>


                    <div class="form-group">
                        <label for="amc_amount">AMC Monthly Amount</label>
                        <input class="form-control" type="text" name="amc_amount" id="amc_amount"
                            value="<?= $vec->amc_amount ?>">
                        <?php if (isset($validation)) { ?><span
                                class="text-danger"><?= $error = $validation->getError('amc_amount'); ?></span><?php } ?>
                    </div>
                    <div class="form-group">
                        <label for="amc_expiry">AMC Expiry Date</label>
                        <input class="form-control" type="text" name="amc_expiry" id="amc_expiry"
                            value="<?= $vec->amc_expary ?>">
                        <?php if (isset($validation)) { ?><span
                                class="text-danger"><?= $error = $validation->getError('amc_expiry'); ?></span><?php } ?>
                    </div>

                    <div class="form-group">
                        <label for="example-nf-email">Amc Frequency</label>
                        <input class="form-control" type="date" name="amc_frequency" id="Permit"
                            value="<?= $vec->amc_frequancy ?>">
                        <?php if (isset($validation)) { ?><span
                                class="text-danger"><?= $error = $validation->getError('amc_frequency'); ?></span><?php } ?>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <?php if (isset($validation)) { ?><span
                                class="text-danger"><?= $error = $validation->getError('pucc'); ?></span><?php } ?>
                    </div>

                    <div class="form-group">
                        <label for="pucc">PUCC Amount</label>
                        <input class="form-control" type="text" name="pucc_amount" id="pucc_amount"
                            value="<?= $vec->pucc_amount ?>">
                        <?php if (isset($validation)) { ?><span
                                class="text-danger"><?= $error = $validation->getError('pucc_amount'); ?></span><?php } ?>
                    </div>


                    <div class="form-group">
                        <label for="i3ms_expiry">I3MS Expiry</label>
                        <input class="form-control" type="date" name="i3ms_expiry" id="i3ms_expiry"
                            value="<?= $vec->i3ms_expary ?>">
                        <?php if (isset($validation)) { ?><span
                                class="text-danger"><?= $error = $validation->getError('i3ms_expiry'); ?></span><?php } ?>
                    </div>
                    <div class="form-group">
                        <label for="i3ms_recharge">I3MS Recharge</label>
                        <input class="form-control" type="text" name="i3ms_recharge" id="i3ms_recharge"
                            value="<?= $vec->i3ms_recharge ?>">
                        <?php if (isset($validation)) { ?><span
                                class="text-danger"><?= $error = $validation->getError('i3ms_recharge'); ?></span><?php } ?>
                    </div>
                    <div class="form-group">
                        <label for="khanij_expiry">Khanij Expiry</label>
                        <input class="form-control" type="date" name="khanij_expiry" id="khanij_expiry"
                            value="<?= $vec->khanij_expiri ?>">
                        <?php if (isset($validation)) { ?><span
                                class="text-danger"><?= $error = $validation->getError('khanij_expiry'); ?></span><?php } ?>
                    </div>

                    <div class="form-group">
                        <label for="khanij_expiry">Khanij Amount</label>
                        <input class="form-control" type="text" name="khanij_amount" id="khanij_amount"
                            value="<?= $vec->khanij_amount ?>">
                        <?php if (isset($validation)) { ?><span
                                class="text-danger"><?= $error = $validation->getError('khanij_amount'); ?></span><?php } ?>
                    </div>
                    ...
                    <div class="uk-margin-bottom">
                        ...
                        <?php if (isset($validation)) { ?><span
                                class="text-danger"><?= $error = $validation->getError('location_name'); ?></span><?php } ?>
                    </div>
    ...
    public function Add_staf()
    {
        ...
        if ($this->validate($rules)) {
            ...
            $firstThreeChars = strtoupper(substr($clean_name, 0, 3));
            $aadhaar_clean = preg_replace('/[^0-9]/', '', $aadhaar_no);
            $aadhaar_last4 = !empty($aadhaar_clean) ? substr($aadhaar_clean, -4) : '0000';
            $staff_code = $firstThreeChars . $aadhaar_last4 . '-' . $lastInsertID;
    ...
    public function PrintStaff()
    {
        ...
        if ($staff) {
            ...
            <body style="font-family: Arial, sans-serif; margin: 0; padding: 0;">
                <div style="width: 80%; margin: 20px auto; border: 1px solid #000; padding: 20px; box-shadow: 0 0 10px rgba(0,0,0,0.1);"
                    id="staff-details">
                    <div
                        style="text-align: center; margin-bottom: 20px; display: flex; align-items: center; justify-content: center;">
                        <img src="<?php echo base_url(); ?>/assets/admin/images/yasujalogo.png" alt="Yasuja Logo"
                            style="height: 50px; margin-right: 10px;">
                        <h2 style="margin: 0; color: red; background-color: lightyellow; padding: 5px;">YASUJA ENTERPRISES PVT. LTD.
                        </h2>
                    </div>
                    ...
                    <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 20px;">
                        ...
                        <div style="flex: 1;  padding: 10px; text-align: center;">
                            <img src="<?php echo base_url(); ?>/uploads/<?= $staff->img ?>" alt="Driver Photo"
                    ...
                    <h4 style="text-align: center; margin-bottom: 10px;  color: red; background-color: lightyellow;">DOCUMENT
                        DETAILS</h4>
                    ...
                    <h4 style="text-align: center; margin-bottom: 10px;  color: red; background-color: lightyellow;">BANK ACCOUNT
                        DETAILS</h4>
                    <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td style="border: 1px solid #000; padding: 10px; text-align: center;"><?= $staff->name_bank; ?>
                                </td>
                            </tr>
                        </tbody>
                    </table>
                    ...
                    <h4 style="text-align: center; margin-bottom: 10px;  color: red; background-color: lightyellow;">PERSONAL
                        DETAILS</h4>
                    <div style="display: flex; flex-direction: column; gap: 10px; margin-bottom: 20px;">
                        <div style="display: flex; align-items: center;">
                            <span style="flex: 1;">FATHER'S NAME:</span>
                            <div style="flex: 2; border: 1px solid black; padding: 5px; text-align: center;">
                                <?= $staff->fathers_name; ?>
                            </div>
                        </div>
                        <div style="display: flex; align-items: center;">
                        ...
                        <div style="display: flex; align-items: center;">
                            ...
                            <div style="flex: 2; border: 1px solid black; padding: 5px; text-align: center;"><?= $staff->dob; ?>
                            </div>
                        </div>
                        <div style="display: flex; align-items: center;">
                        ...
                        <div style="display: flex; align-items: center;">
                            ...
                            <div style="flex: 2; border: 1px solid black; padding: 5px; text-align: center;">
                                <?= $staff->family_contact; ?>
                            </div>
                        </div>
                        <div style="display: flex; align-items: center;">
                            <span style="flex: 1;">ADDRESS:</span>
                            <div style="flex: 2; border: 1px solid black; padding: 5px; text-align: center;"><?= $staff->address; ?>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-6">
                            ...
                            <div class="uk-margin">
                                ...
                                <select class="form-control" name="user_type" aria-label="Select">
                                    ...
                                    <option value="MECHANIC" <?= $staff->user_type == 'MECHANIC' ? 'selected' : ''; ?>>Mechanic
                                    </option>
                                </select>
                        ...
                        <div class="col-sm-6">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="name_bank" placeholder="Enter your name in bank"
                                    value="<?= $staff->name_bank; ?>">
                        ...
                        <div class="col-sm-6">
                            <div class="form-group">
                                <label>A/c No</label>
                                <input type="number" class="form-control" name="ac_no" placeholder="Enter account number"
                                    value="<?= $staff->ac_no; ?>">
                                <?php if (isset($validation)) { ?><span
                                        class="text-danger"><?= $error = $validation->getError('ac_no'); ?></span><?php } ?>
                            </div>
                        </div>
                        <div class="col-sm-6">
                            <div class="form-group">
                                <label>IFSC Code</label>
                                <input type="text" class="form-control" name="ifsc" placeholder="Enter your IFSC code"
                                    value="<?= $staff->ifsc; ?>">
                        ...
                        <div class="col-sm-6">
                            <div class="form-group">
                                ...
                                <input type="file" class="form-control" name="dl_front" placeholder="Upload your DL picture"
                                    value="<?= $staff->dl_front; ?>">
                        ...
                        <div class="col-sm-6">
                            <div class="form-group">
                                <label>DL Back</label>
                                <input type="file" class="form-control" name="dl_back" placeholder="Upload your DL picture"
                                    value="<?= $staff->dl_back; ?>">
                        ...
                        <div class="col-sm-6">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="dl_number" placeholder="Enter DL number "
                                    value="<?= $staff->dl_number; ?>">
                        ...
                        <div class="col-sm-6">
                            <div class="form-group">
                                ...
                                <input type="date" class="form-control" name="dl_expiry" placeholder="Enter DL expiry date"
                                    value="<?= $staff->dl_expiry; ?>">
                        ...
                        <div class="col-sm-6">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="aadhaar_no" placeholder=" Aadhaar Number"
                        ...
                        <div class="col-sm-6">
                            <div class="form-group">
                                ...
                                <input type="file" class="form-control" name="aadhaar_front" placeholder="Upload Aadhaar"
                                    value="<?= $staff->aadhaar_front; ?>">
                        ...
                        <div class="col-sm-6">
                            <div class="form-group">
                                ...
                                <input type="file" class="form-control" name="aadhaar_back" placeholder="Upload Aadhaar"
                                    value="<?= $staff->aadhaar_back; ?>">
                        ...
                        <div class="col-sm-6">
                            <div class="form-group">
                                ...
                                <input type="tel" class="form-control" name="tel" placeholder="Enter your Contact Number"
                                    value="<?= $staff->tel; ?>">
                        ...
                        <div class="col-sm-6">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="fathers_name" placeholder="Enter your Father's Name"
                                    value="<?= $staff->fathers_name; ?>">
                        ...
                        <div class="col-sm-6">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="spouse_name" placeholder="Enter your Spouse Name"
                        ...
                        <div class="col-sm-6">
                            <div class="form-group">
                                ...
                                <input type="tel" class="form-control" name="family_contact"
                                    placeholder="Enter your Family Contact Number" value="<?= $staff->family_contact; ?>">
                        ...
                        <div class="col-sm-6">
                            <div class="form-group">
                                ...
                                <select name="location_id" id="location" class="form-control">
                                    <option value="">Select location</option>
                                    <?php foreach ($location as $loc): ?>
                                        <option <?= ($loc->location_id == $staff->location_id || (empty($staff->location_id) && $loc->location_id == $staff->address)) ? 'selected' : ''; ?>
                                            value="<?= $loc->location_id; ?>"><?= $loc->location_name; ?></option>
                                    <?php endforeach; ?>
                                </select>
    ...
    public function upload_staff_excel()
    {
        ...
        if ($file->isValid() && !$file->hasMoved()) {
            ...
            // Read with raw data (formatData = false) to get accurate date serial numbers
            ...
            foreach ($data as $row) {
                ...
                $location_name = $row['T'] ?? null;
                $location_id = null;

                if (!empty($location_name)) {
                    $loc = $this->db->table('location')
                        ->like('location_name', trim($location_name), 'both')
                        ->get()
                        ->getRow();
                ...
                $rawAadhaar = $row['K'] ?? '';
                if (is_numeric($rawAadhaar)) {
                    $aadhaar = sprintf('%.0f', $rawAadhaar);
                } else {
                    $aadhaar = preg_replace('/[^0-9]/', '', $rawAadhaar);
                }

                $staffData = [
                    'user_type' => strtoupper($row['B'] ?? 'STAFF'),
                    'name' => $row['C'] ?? '',
                    'doj' => $parseDate($row['D'] ?? null),
                    'salary' => $row['E'] ?? 0,
                    'name_bank' => $row['F'] ?? '',
                    'ac_no' => $row['G'] ?? '',
                    'ifsc' => $row['H'] ?? '',
                    'dl_number' => $row['I'] ?? '',
                    'dl_expiry' => $parseDate($row['J'] ?? null),
                    'aadhaar_no' => $aadhaar,
                    'pan_no' => strtoupper(trim($row['L'] ?? '')),
                    'tel' => $row['M'] ?? '',
                    'fathers_name' => $row['N'] ?? '',
                    'spouse_name' => $row['O'] ?? '',
                    'dob' => $parseDate($row['P'] ?? null),
                    'family_contact' => $row['Q'] ?? '',
                    'blood_group' => $row['R'] ?? '',
                ...
                $clean_name = preg_replace('/[^a-zA-Z]/', '', $row['C']);
                $firstThreeChars = substr($clean_name, 0, 3);

                $aadhaar_last4 = !empty($aadhaar) ? substr($aadhaar, -4) : '0000';

                $staff_code = strtoupper($firstThreeChars) . $aadhaar_last4 . '-' . $lastInsertID;

                $this->db->table('staff')->where('id', $lastInsertID)
            ...
            session()->setFlashdata('msg', "$successCount staff records imported successfully.");
            return redirect()->to('admin/staf');
        ...
        <table class="table">
            ...
            foreach ($vendor_rate as $vr) { ?>
                <tr>
                    ...
                    <td><a href="<?= base_url(); ?>/Admin/delete_vendor_rate/<?= $vr->vendor_rate_id ?>"
                    ...
                    <div class="row">
                        <div class="col-sm-6">
                            <div class="form-group">
                                ...
                                <?php if (isset($validation)) { ?><span
                                        class="text-danger"><?= $validation->getError('name'); ?></span><?php } ?>
                            </div>
                        </div>
                        ...
                        <div class="col-sm-6">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="gst" name="gst" placeholder="Enter GST number"
                                    value="<?= htmlspecialchars($vendor->gst) ?>">
                                <?php if (isset($validation)) { ?><span
                                        class="text-danger"><?= $validation->getError('gst'); ?></span><?php } ?>
                            </div>
                        </div>
                        ...
                        <div class="col-sm-6">
                            <div class="form-group">
                                ...
                                <?php if (isset($validation)) { ?><span
                                        class="text-danger"><?= $error = $validation->getError('location'); ?></span><?php } ?>
                            </div>
                        </div>
                        ...
                        <div class="col-sm-6">
                            ...
                            <?php if (isset($validation)) { ?><span
                                    class="text-danger"><?= $validation->getError('type'); ?></span><?php } ?>
                        </div>

                        <div class="col-sm-6">
                            <div class="form-group">
                                <label>PAN No</label>
                                <input type="text" class="form-control" id="panNo" name="pan" placeholder="Enter your PAN No."
                                    value="<?= htmlspecialchars($vendor->pan) ?>">
                                <?php if (isset($validation)) { ?><span
                                        class="text-danger"><?= $validation->getError('pan'); ?></span><?php } ?>
                            </div>
                        </div>
                        ...
                        <div class="col-sm-6">
                            <div class="form-group">
                                ...
                                <input type="number" class="form-control" id="bal" name="bal"
                                    placeholder="Enter your opening balance" value="<?= htmlspecialchars($vendor->bal) ?>">
                                <?php if (isset($validation)) { ?><span
                                        class="text-danger"><?= $validation->getError('bal'); ?></span><?php } ?>
                            </div>
                        </div>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="uk-button-group">
                                        ...
                                        <a href="javascript:void(0);" onClick="vendor_rate('<?= $vendor->id; ?>');"
                                            class="uk-button uk-button-small uk-button-primary">
                                            view rate
                                        </a>
                                        <a href="<?= base_url('Admin/deletevendor/' . $vendor->id); ?>"
    ...
    function edit_item_data()
    {
        ...
        <form action="<?php echo base_url(); ?>/Admin/update_items" enctype="multipart/form-data" method="post">
            <div class="uk-margin-bottom">
                ...
                <input type="text" name="item_code" placeholder="enter Item code" id="item_id" class="form-control"
                    value="<?= $itm->item_id ?>" />
                <?php if (isset($validation)) { ?><span
                        class="text-danger"><?= $error = $validation->getError('item_id'); ?></span><?php } ?>
            </div>
            <div class="uk-margin-bottom">
                <lable>Item Name </lable>
                <input type="hidden" name="item_id" value="<?= $itm->id ?>" />
                <input type="text" name="item_name" placeholder="enter Item Name" id="item_name" class="form-control"
                    value="<?= $itm->item_name ?>" />
                <?php if (isset($validation)) { ?><span
                        class="text-danger"><?= $error = $validation->getError('item_name'); ?></span><?php } ?>
            </div>
            <div class="uk-margin-bottom">
                ...
                <?php if (isset($validation)) { ?><span
                        class="text-danger"><?= $error = $validation->getError('unit_of_measurement'); ?></span><?php } ?>
            </div>
            <div class="uk-margin-bottom">
                <lable>Avg Price Rate</lable>
                <input type="number" name="avg_price_rate" placeholder="enter avg. price rate" id="avg_price_rate"
                    class="form-control" value="<?= $itm->avg_price_rate ?>" />
                <?php if (isset($validation)) { ?><span
                        class="text-danger"><?= $error = $validation->getError('avg_price_rate'); ?></span><?php } ?>
            </div>
            <div class="uk-margin-bottom">
                <lable>Amount</lable>
                <input type="text" name="amount" placeholder="enter  amount " id="amount" class="form-control"
                    value="<?= $itm->amount ?>" />
                <?php if (isset($validation)) { ?><span
                        class="text-danger"><?= $error = $validation->getError('amount'); ?></span><?php } ?>
            </div>
            <!--    <div class="uk-margin-bottom">-->
            ...
            <div class="uk-margin-bottom">
                ...
                <input type="file" name="upload_photo" placeholder="Upload ur Photo" id="upload_photo" class="form-control"
                    value="<?= set_value('upload_photo') ?>" />
                <?php if (isset($validation)) { ?><span
                        class="text-danger"><?= $error = $validation->getError('upload_photo'); ?></span><?php } ?>
            </div>
            <div class="uk-margin-bottom">
                <button type="submit" class="btn btn-primary">Submit</button>
            </div>
            ...
            <tbody id="TextBoxContainer">
                ...
                foreach ($cart_dtls as $cart) {
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="hidden" class="uk-input uk-form-small cart-data" name="cart_id" style="width:50px"
                                value="<?= $cart->cart_id; ?>" />
                            <input type="number" class="uk-input uk-form-small cart-data" name="qty" style="width:50px"
                                value="<?= $cart->qty; ?>" />
                        </td>
                        <td><?= $cart->unit_name; ?></td>
                        <td><input type="text" class="uk-input uk-form-small cart-data" name="rate" style="width:200px"
                                value="<?= $cart->rate; ?>" /></td>
                        <td><?php echo $cart->rate * $cart->qty; ?></td>

                        <td><a href="javascript:void(0);" onClick="deleteRecord('<?= $cart->cart_id; ?>');" uk-icon="icon: trash"
                                class="uk-text-danger"></a></td>
                    </tr>
                    <?php
    ...
    function staf_advance()
    {
        if ($this->session->get('user_id')) {
            ...
            $data['admin_users'] = $this->db->table('user')->whereIn('user_type', [1, 2])->where('deleted_by', null)->get()->getResult();
            $data['date'] = [
                'from_date' => $from_date,
                'to_date' => $to_date,
            ];
    ...
    function insert_staf_advance()
    {
        ...
        if ($data['bank_cash'] == 'Cash') {
            $staff = $this->db->query("SELECT name, user_type FROM staff WHERE id = ?", [$data['staff_id']])->getRow();
            ...
            $group_id = ($staff->user_type == 'STAFF') ? 5 : 4; // Staff=5, Driver=4
            ...
            $voucherData = [
                ...
                'narration' => "Advance paid to " . ($staff->name ?? 'Staff') . " at " . ($location->location_name ?? 'Location') . ". Paid by: " . $data['paid_by'],
        ...
        if ($file->isValid() && !$file->hasMoved()) {
            ...
            try {
                ...
                foreach ($sheetData as $index => $rowData) {
                    ...
                    $staff_dtl = $this->db->query("SELECT * FROM staff WHERE staff_code = ?", [$rowData[0] ?? ''])->getFirstRow();
                    if (!$staff_dtl) {
                        $is_valid = false;
                    }

                    $loc_name = $rowData[5] ?? '';
                    $loc_dtl = $this->db->query("SELECT * FROM location WHERE location_name = ?", [$loc_name])->getFirstRow();
                    if (!$loc_dtl) {
                        $is_valid = false;
                    }
                    ...
                    if ($is_valid) {
                        $insertData['staff_id'] = $staff_dtl->id;
                        $this->db->table('staff_advance')->insert($insertData);
                        $insertID = $this->db->insertID();

                        // Account Voucher Insertion for Excel Upload
                        if ($insertData['bank_cash'] == 'Cash' || $insertData['bank_cash'] == 'CASH') {
                            $fy = $this->db->query("SELECT fy_id FROM financial_year WHERE status = 1")->getRow();
                            $fy_id = $fy->fy_id ?? 0;

                            $group_id = ($staff_dtl->user_type == 'STAFF') ? 5 : 4;

                            $entries = [
                                [
                                    'group_id' => $group_id,
                                    'ledger_id' => $staff_dtl->id,
                                    'entry_type' => 1,
                                    'amount' => $insertData['amount'],
                                    'narration' => "Advance received (Excel). [Adv ID: " . $insertID . "]"
                                ],
                                [
                                    'group_id' => 2,
                                    'ledger_id' => $loc_dtl->location_id,
                                    'entry_type' => 2,
                                    'amount' => $insertData['amount'],
                                    'narration' => "Advance paid to " . ($staff_dtl->name ?? 'Staff')
                                ]
                            ];

                            $voucherData = [
                                'voucher_no' => $this->AdminModel->getNextVoucherNo('Payment'),
                                'voucher_date' => $insertData['adv_date'],
                                'voucher_type' => 'Payment',
                                'fy_id' => $fy_id,
                                'location' => $loc_dtl->location_id,
                                'total_amount' => $insertData['amount'],
                                'narration' => "Advance paid to " . ($staff_dtl->name ?? 'Staff') . " at " . ($loc_dtl->location_name ?? 'Location') . " (Excel Import)",
                                'created_at' => date('Y-m-d H:i:s'),
                                'created_by' => $this->session->get('user_id')
                            ];
                            $this->AdminModel->saveVoucher($voucherData, $entries);
                        ...
                        $insertData['staff_name'] = $rowData[1] ?? '';
                        $this->db->table('staff_advance1')->insert($insertData);
                    }
                }
    ...
    public function CashBank()
    {
        ...
        if ($location_id) {
            ...
            // 3. Accounting Vouchers (Direct entries from Cash Book group)
            $vouchers = $this->db->query("
                SELECT av.voucher_date as date, 
                       (SELECT GROUP_CONCAT(DISTINCT 
                            CASE 
                                WHEN ave3.group_id = 6 THEN CONCAT('Vendor: ', v.name)
                                WHEN ave3.group_id = 5 THEN CONCAT('Staff: ', s.name)
                                WHEN ave3.group_id = 4 THEN CONCAT('Driver: ', s2.name)
                                WHEN ave3.group_id = 3 THEN CONCAT('Vehicle: ', veh.vehicle_no)
                                WHEN ave3.group_id = 2 THEN CONCAT('Cash: ', l.location_name)
                                ELSE 'Other'
                            END SEPARATOR ', ')
                        FROM account_voucher_entries ave3 
                        LEFT JOIN vendor v ON v.id = ave3.ledger_id AND ave3.group_id = 6
                        LEFT JOIN staff s ON s.id = ave3.ledger_id AND ave3.group_id = 5
                        LEFT JOIN staff s2 ON s2.id = ave3.ledger_id AND ave3.group_id = 4
                        LEFT JOIN vehicle veh ON veh.id = ave3.ledger_id AND ave3.group_id = 3
                        LEFT JOIN location l ON l.location_id = ave3.ledger_id AND ave3.group_id = 2
    ...
    function get_driver_report_details()
    {
        ...
        // Group data by truck_no
        $grouped_data = [];
        $grand_total_trips = 0;
        $grand_total_expense = 0;
        $grand_total_cash = 0;
        $grand_total_diesel = 0;

        foreach ($all_data as $row) {
            $truck = $row['truck_no'];
            if (!isset($grouped_data[$truck])) {
                $grouped_data[$truck] = [
                    'truck_no' => $truck,
                    'opening_hsd' => $row['opening_hsd'],
                    'opening_km' => $row['opening_km'],
                    'closing_hsd' => $row['closing_hsd'],
                    'closing_km' => $row['closing_km'],
                    'data' => []
                ];
            }
            $grouped_data[$truck]['data'][] = $row;
            // Update closing balances to the last row's balances for that vehicle
            $grouped_data[$truck]['closing_hsd'] = $row['closing_hsd'];
            $grouped_data[$truck]['closing_km'] = $row['closing_km'];
        }
        ?>
        <div class="table-responsive">
            ...
            <?php if (!empty($grouped_data)): ?>
                <?php foreach ($grouped_data as $truck_no => $group):
                    $visible_rows = array_filter($group['data'], function ($r) {
                        return $r['trips'] > 0;
                    });
                    $visible_rows = array_values($visible_rows);
                    ...
                    <div class="card mb-4">
                        <div class="card-body p-0">
                            <table class="uk-table uk-table-striped uk-table-small mb-0" style="width:100%">
                                <thead>
                                    <tr>
                                        <th>Date</th>
                                        <th>Truck No.</th>
                                ...
                                <tbody>
                                    ...
                                    foreach ($visible_rows as $idx => $row) {
                                        ...
                                        // Grand totals
                                        $grand_total_trips += $row['trips'];
                                        $grand_total_expense += $row['accrued_expense'];
                                        $grand_total_cash += $row['cash_paid'];
                                        $grand_total_diesel += $row['diesel_issued'];
                                        ?>
                                        <tr>
                                            <td><?= $row['date']; ?></td>
                                            <td><?= $row['truck_no']; ?></td>
                                            <td><?= ($idx === 0) ? number_format($row['opening_hsd'], 2) : ''; ?></td>
                                            <td><?= ($idx === $row_count - 1) ? number_format($row['closing_hsd'], 2) : ''; ?></td>
                                            <td><?= $row['trips']; ?></td>
                                    ...
                                    <?php /* ?>
                                                                     <tr style="font-weight: bold; background: #f8f9fa;">
                                                                         <td colspan="5">Total for <?= $truck_no ?></td>
                                                                         <td><?= $total_trips; ?></td>
                ...
                <div class="mt-4 p-3 border">
                    <div class="row" style="font-weight: bold; font-size: 1.1em;">
                        ...
                        <div class="col-md-2">Trips: <?= $grand_total_trips; ?></div>
                        <div class="col-md-2">Expenses: <?= number_format($grand_total_expense, 2); ?></div>
                        <div class="col-md-2">Cash: <?= number_format($grand_total_cash, 2); ?></div>
                        <div class="col-md-3 text-end">Diesel: <?= number_format($grand_total_diesel, 2); ?></div>
                    </div>
                </div>
            <?php else: ?>
    ...
    function Vehicle_Report()
    {
        if ($this->session->get('user_id')) {
            ...
            $data['setting'] = $this->AdminModel->Settingdata();
            $data['singleuser'] = $this->AdminModel->userdata($user_id);
            $data['vehicles'] = $this->db->table('vehicle')->get()->getResult();
            return view('admin/vehicle_report_vw', $data);
        } else {
            return redirect()->to('Admin/');
        }
    }

    function get_vehicle_report_details()
    {
        $vehicle_id = $this->request->getVar('vehicle');
        $from_date = $this->request->getVar('from_date');
        $to_date = $this->request->getVar('to_date');

        if (empty($vehicle_id) || empty($from_date) || empty($to_date)) {
            return $this->response->setStatusCode(400)->setBody("Please select Vehicle and Date Range.");
        }

        $all_data = $this->AdminModel->get_vehicle_daily_report_data($vehicle_id, $from_date, $to_date);

        $all_data = $this->AdminModel->get_vehicle_daily_report_data($vehicle_id, $from_date, $to_date);

        // Group data by driver assignment stints
        $stints = [];
        $current_stint = null;
        $grand_total_trips = 0;
        $grand_total_expense = 0;
        ...
        $visible_rows_all = array_filter($all_data, function ($r) {
            return $r['trips'] > 0;
        });
        $visible_rows_all = array_values($visible_rows_all);

        foreach ($visible_rows_all as $row) {
            $driver_key = $row['driver_name'] . '|' . $row['truck_no'];

            if ($current_stint === null || $current_stint['key'] !== $driver_key) {
                if ($current_stint !== null) {
                    $stints[] = $current_stint;
                }
                $current_stint = [
                    'key' => $driver_key,
        ...
        <div class="table-responsive">
            ...
            <div class="report-container"
                ...
                <?php if (!empty($stints)): ?>
                    <?php foreach ($stints as $stint):
                        $rows = $stint['data'];
                        $row_count = count($rows);
                        ?>
                        <table class="uk-table uk-table-divider uk-table-small mb-4" style="width:100%">
                            <thead>
                                <tr style="background: #fff;">
                                    <th
                                        style="color: #bbb; font-size: 11px; font-weight: 600; text-transform: uppercase; border-bottom: 1px solid #f0f0f0;">
                                        Date</th>
                                    <!-- <th style="color: #bbb; font-size: 11px; font-weight: 600; text-transform: uppercase; border-bottom: 1px solid #f0f0f0;">Truck No</th> -->
                                    <th
                                        style="color: #bbb; font-size: 11px; font-weight: 600; text-transform: uppercase; border-bottom: 1px solid #f0f0f0;">
                                        Opening Diesel</th>
                                    <th
                            ...
                            <tbody>
                                <?php foreach ($rows as $idx => $row): ?>
                                    <tr>
                                        <td style="font-size: 13px; color: #444;"><?= $row['date']; ?></td>
                                        <td style="font-size: 13px; color: #444;">
                                            <?= ($idx === 0) ? number_format($row['opening_hsd'], 2) : ''; ?>
                                        </td>
                                        <td style="font-size: 13px; color: #444;">
                                            <?= ($idx === $row_count - 1) ? number_format($row['closing_hsd'], 2) : ''; ?>
                                        </td>
                                        <td style="font-size: 13px; color: #444;"><?= $row['driver_name']; ?></td>
                                        <td style="font-size: 13px; color: #444;"><?= $row['trips']; ?></td>
                                        <td style="font-size: 13px; color: #444;"><?= number_format($row['accrued_expense'], 2); ?></td>
                                        <td style="font-size: 13px; color: #444;"><?= number_format($row['cash_paid'], 2); ?></td>
                                        <td style="font-size: 13px; color: #444;"><?= number_format($row['diesel_issued'], 2); ?></td>
                                        <td style="font-size: 13px; color: #444;"><?= $row['remarks']; ?></td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
                        </table>
                    ...
                    <div class="grand-total-bar mt-4 p-3 d-flex justify-content-between align-items-center"
                        style="border: 1px solid #eee; border-radius: 4px; background: #fff;">
                        <span style="font-weight: 700; color: #555; text-transform: uppercase;">Grand Total:</span>
                        <div class="d-flex gap-5">
                            <span style="font-weight: 600;">Trips: <?= $grand_total_trips; ?></span>
                            <span style="font-weight: 600;">Expenses: <?= number_format($grand_total_expense, 2); ?></span>
                            <span style="font-weight: 600;">Cash: <?= number_format($grand_total_cash, 2); ?></span>
                            <span style="font-weight: 600;">Diesel: <?= number_format($grand_total_diesel, 2); ?></span>
                        </div>
                    </div>

                <?php else: ?>
                    <div class="alert alert-info py-2">No trip data found for the selected criteria.</div>
                <?php endif; ?>
            </div>
        </div>
        <?php
    ...
    public function get_driver_report_excel()
    {
        ...
        $row_idx = 4;
        foreach ($grouped_data as $truck_no => $group) {
            $visible_rows = array_filter($group['data'], function ($r) {
                return $r['trips'] > 0;
            });
            $visible_rows = array_values($visible_rows);
            ...
            // Table Headers
            $headers = ['Date', 'Truck No.', 'Opening Diesel', 'Closing Diesel', 'No of Trips', 'Trip Expenses Accrued', 'Cash Paid', 'Diesel Issued', 'Remarks'];
            $col = 'A';
            foreach ($headers as $header) {
            ...
            foreach ($visible_rows as $idx => $row) {
                ...
                $sheet->setCellValue('C' . $row_idx, ($idx === 0) ? (float) $row['opening_hsd'] : '');
                $sheet->setCellValue('D' . $row_idx, ($idx === $row_count - 1) ? (float) $row['closing_hsd'] : '');
                $sheet->setCellValue('E' . $row_idx, $row['trips']);
                $sheet->setCellValue('F' . $row_idx, (float) $row['accrued_expense']);
                $sheet->setCellValue('G' . $row_idx, (float) $row['cash_paid']);
                $sheet->setCellValue('H' . $row_idx, (float) $row['diesel_issued']);
                $sheet->setCellValue('I' . $row_idx, $row['remarks']);
                ...
                // Grand totals
                $grand_total_trips += $row['trips'];
                $grand_total_expense += (float) $row['accrued_expense'];
                $grand_total_cash += (float) $row['cash_paid'];
                $grand_total_diesel += (float) $row['diesel_issued'];

                $row_idx++;
            }

            /*
            // Total Row for Vehicle
            $sheet->setCellValue('A' . $row_idx, 'Total for ' . $truck_no);
            $sheet->setCellValue('D' . $row_idx, $total_trips);
            $sheet->setCellValue('E' . $row_idx, $total_expense);
            $sheet->setCellValue('F' . $row_idx, $total_cash);
            $sheet->setCellValue('G' . $row_idx, $total_diesel);
            ...
            */
            ...
            /*
            // Vehicle Footer
            $sheet->setCellValue('A' . $row_idx, 'Closing Diesel: ' . $group['closing_hsd']);
            $sheet->setCellValue('E' . $row_idx, 'Closing KM: ' . $group['closing_km']);
            $sheet->getStyle('A' . $row_idx . ':E' . $row_idx)->getFont()->setBold(true);
            $row_idx += 2; // Add some space before next vehicle
            */
            $row_idx++; // Minimal space
        }

        // Grand Total Row
        $sheet->setCellValue('A' . $row_idx, 'Grand Total');
        $sheet->setCellValue('E' . $row_idx, $grand_total_trips);
        $sheet->setCellValue('F' . $row_idx, $grand_total_expense);
        $sheet->setCellValue('G' . $row_idx, $grand_total_cash);
        $sheet->setCellValue('H' . $row_idx, $grand_total_diesel);
    ...
    public function get_vehicle_report_excel()
    {
        $vehicle_id = $this->request->getVar('vehicle');
        $from_date = $this->request->getVar('from_date');
        $to_date = $this->request->getVar('to_date');

        $vehicle = $this->db->table('vehicle')->where('id', $vehicle_id)->get()->getRow();
        $all_data = $this->AdminModel->get_vehicle_daily_report_data($vehicle_id, $from_date, $to_date);

        $spreadsheet = new \PhpOffice\PhpSpreadsheet\Spreadsheet();
        $sheet = $spreadsheet->getActiveSheet();

        $sheet->setCellValue('A1', 'Vehicle Report');
        $sheet->setCellValue('A2', 'Vehicle No: ' . $vehicle->vehicle_no);
        $sheet->setCellValue('A3', 'Period: ' . $from_date . ' to ' . $to_date);
        ...
        foreach ($stints as $stint) {
            ...
            // Table Headers
            $headers = ['Date', 'Opening Diesel', 'Closing Diesel', 'Driver Name', 'No of Trips', 'Trip Expenses Accrued', 'Cash Paid', 'Diesel Issued', 'Remarks'];
            $col = 'A';
            foreach ($headers as $header) {
                $sheet->setCellValue($col . $row_idx, $header);
                $sheet->getStyle($col . $row_idx)->getFont()->setBold(true);
            ...
            foreach ($rows as $idx => $row) {
                $sheet->setCellValue('A' . $row_idx, $row['date']);
                $sheet->setCellValue('B' . $row_idx, ($idx === 0) ? (float) $row['opening_hsd'] : '');
                $sheet->setCellValue('C' . $row_idx, ($idx === $row_count - 1) ? (float) $row['closing_hsd'] : '');
                $sheet->setCellValue('D' . $row_idx, $row['driver_name']);
                $sheet->setCellValue('E' . $row_idx, $row['trips']);
                $sheet->setCellValue('F' . $row_idx, (float) $row['accrued_expense']);
                $sheet->setCellValue('G' . $row_idx, (float) $row['cash_paid']);
                $sheet->setCellValue('H' . $row_idx, (float) $row['diesel_issued']);
            ...
            $row_idx++; // Minimal space between stints
        }

        // Grand Total Row (matches the bar structure)
        $sheet->setCellValue('A' . $row_idx, 'GRAND TOTAL:');
        $sheet->setCellValue('E' . $row_idx, 'Trips: ' . $grand_total_trips);
        $sheet->setCellValue('F' . $row_idx, 'Expenses: ' . number_format($grand_total_expense, 2));
        $sheet->setCellValue('G' . $row_idx, 'Cash: ' . number_format($grand_total_cash, 2));
        $sheet->setCellValue('H' . $row_idx, 'Diesel: ' . number_format($grand_total_diesel, 2));

        $sheet->getStyle('A' . $row_idx . ':I' . $row_idx)->getFont()->setBold(true);

        $filename = "Vehicle_Report_" . ($vehicle ? str_replace(' ', '_', $vehicle->vehicle_no) : 'Report') . ".xlsx";
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment;filename="' . $filename . '"');
        header('Cache-Control: max-age=0');
    ...
    function getdriver_salary_details()
    {
        ...
        if (!empty($staff_ids)) {
            $all_advances = $this->db->table('staff_advance')
                ...
                ->get()->getResult();
            foreach ($all_advances as $row) {
                $advances_by_staff[$row->staff_id][] = $row;
            }
            ...
            $all_adjustments = $this->db->table('adjust_salary')
                ...
                ->get()->getResult();
            foreach ($all_adjustments as $row) {
                $adjustments_by_staff[$row->driver_id][] = $row;
            }
            ...
            foreach ($all_payments as $row) {
                $payments_by_staff[$row->driver_id] = $row;
            }
        }
                        ...
                        <tr>
                            <td>
                                <input type="checkbox" class="driver-select-cb"
                                    ...
                                    data-name="<?= htmlspecialchars($staf->name ?? '') ?>" data-staff-id="<?= $staf->id ?>"
                                    data-year="<?= $year ?>" data-month="<?= $month ?>"
                                    data-from-date="<?= $staf->from_date ?? '' ?>" data-to-date="<?= $staf->to_date ?? '' ?>">
                            </td>
                            <td><?= $i++; ?></td>
                            ...
                            <td>(<?= number_format($total_d_req, 2) ?> - <?= number_format($used_hsd, 2) ?>) ===
                                <?= number_format($HSD_LTR, 2); ?>
                            </td>
                            ...
                            <td>
                                <button class="btn btn-primary btn-sm"
                                    onclick='openPaymentModal("<?= $staf->id ?>", "<?= $staf->name ?>", "<?= $tsalary ?>", "<?= $display_opening ?>")'>
                                    Payment
                                </button>
                                <a href="<?= base_url('Admin/salary_slip?staff_id=' . $staf->id . '&year=' . $year . '&month=' . $month) ?>"
    ...
    function getdriver_salary_details_excel()
    {
        ...
        if (!empty($staff_ids)) {
            $all_advances = $this->db->table('staff_advance')
                ...
                ->where('adv_date <=', $last_datesam)
                ->get()->getResult();
            foreach ($all_advances as $rowAdv) {
                $advances_by_staff[$rowAdv->staff_id][] = $rowAdv;
            }
            ...
            $all_adjustments = $this->db->table('adjust_salary')
                ...
                ->where('from_date <=', $last_datesam)
                ->get()->getResult();
            foreach ($all_adjustments as $rowAdj) {
                $adjustments_by_staff[$rowAdj->driver_id][] = $rowAdj;
            }
            ...
            $all_payments = $this->db->table('salary_payment')
                ...
                ->where('year', (int) $year)
                ->get()->getResult();
            foreach ($all_payments as $rowPay) {
                $payments_by_staff[$rowPay->driver_id] = $rowPay;
            }
        }
    ...
    public function send_salary_whatsapp_bulk()
    {
        ...
        $api_url = "https://api.ultramsg.com/{$instance_id}/messages/document";
        $results = [];
        $success_cnt = 0;
        $fail_cnt = 0;
        ...
        foreach ($drivers as $drv) {
            ...
            $year = $drv['year'] ?? date('Y');
            $month = $drv['month'] ?? date('m');
            ...
            // PDF URL (must be public accessible)
            $pdf_url = base_url("salary-pdf/{$staff_id}?year={$year}&month={$month}&from_date={$from_date}&to_date={$to_date}");
        ...
        return $this->response->setJSON([
            ...
            'message' => "Sent: {$success_cnt}, Failed: {$fail_cnt}",
            'sent' => $success_cnt,
            'failed' => $fail_cnt,
            'details' => $results
        ]);
    ...
    public function salary_pdf($staff_id)
    {
        $db = \Config\Database::connect();

        $staff = $db->table('staff')->where('id', $staff_id)->get()->getRow();
        ...
        $html = view('salary_pdf_template', $data);
        ...
        return $dompdf->stream("salary-slip.pdf", ["Attachment" => true]);
    }
    ...
    //     // ── Raw Despatch Records (for Trip Details table) ─────────────────────
    //     $trips = $this->db->table('despatch')
    //         ->select('
    //             despatch.*,
    //             vehicle.vehicle_no             AS reg_no,
    //             route.to_city                  AS trip_location,
    //             route.from_city                AS from_city,
    //             route.location_shortname       AS route_name,
    //             do_registration.diesel_type    AS diesel_required,
    //             do_registration.do_no          AS do_reg_no
    //         ')
    ...
    //     // ── DO-wise Grouping (done in controller, passed to view) ─────────────
    //     $do_grouped = [];
    //     foreach ($trips as $trip) {
    //         $do_key = $trip->do_no; // internal FK

    //         if (!isset($do_grouped[$do_key])) {
    //             $route_label = ($trip->from_city && $trip->trip_location)
    //                 ? $trip->from_city . ' to ' . $trip->trip_location
    //                 : ($trip->trip_location ?: ($trip->route_name ?: '-'));
    ...
    //         $do_grouped[$do_key]['trip_count']++;

    //         // Unique vehicles per DO
    //         if (!in_array($trip->reg_no, $do_grouped[$do_key]['vehicles'])) {
    //             $do_grouped[$do_key]['vehicles'][] = $trip->reg_no;
    //         }

    //         // Trip dates for tooltip
    ...
    //         'trips'               => $trips,          // raw records (for total count etc.)
    //         'do_grouped'          => $do_grouped,     // DO-wise grouped data for table
    ...
    public function salary_slip()
    {
        ...
        foreach ($hsd_details as $hsd_row) {
            ...
            if ($diesel_rate == 0)
                $diesel_rate = (float) $hsd_row->diesel_rate;
        }
        ...
        foreach ($diesel_entries_raw as $de) {
            ...
            if (!isset($diesel_by_vehicle[$vid])) {
                ...
                $diesel_by_vehicle[$vid] = [
                    'vehicle_no' => $veh_row->vehicle_no ?? ('Vehicle #' . $vid),
                    'opening_diesel' => $assignment_row ? (float) $assignment_row->opening_hsd : 0,
                    'closing_diesel' => $assignment_row ? (float) $assignment_row->closing_hsd : 0,
                    'entries' => [],
                    'total_filled' => 0,
                ];
            }
        ...
        $adjust_sum = 0;
        foreach ($adjustments as $adj) {
            if (stripos($adj->remark, 'bonus') !== false)
                $bonus_sum += (float) $adj->amount;
            else
                $adjust_sum += (float) $adj->amount;
        }
        ...
        $data = [
            ...
            'diesel_by_vehicle' => $diesel_by_vehicle,    // vehicle-wise grouped (opening/closing from driver_assignment)
            'setting' => $this->AdminModel->Settingdata(),
        ];
    ...
    public function insert_diesel()
    {
        ...
        if ($this->validate($rules)) {
            ...
            $data['vendor'] = $this->AdminModel->Get_vendor();
            $data['vehicle'] = $this->AdminModel->Getvehicle();
            $data['diesel'] = $this->AdminModel->dieseldata('', '');
    ...
    public function get_vehicle_driver()
    {
        ...
        $assignment = $this->db->table('driver_assignment')
            ->where('vehicle_no', $vehicle_id)
            ->where('(to_date IS NULL OR to_date = "0000-00-00" OR to_date >= CURDATE())')
            ->orderBy('id', 'DESC')
            ->get()->getRow();

        if ($assignment) {
            return $this->response->setJSON(['status' => 'success', 'driver_id' => $assignment->driver]);
        } else {
            return $this->response->setJSON(['status' => 'error', 'message' => 'No driver assigned']);
        }
    }
    ...
    public function get_driver_vehicle()
    {
        $driver_id = $this->request->getPost('driver_id');
        $assignment = $this->db->table('driver_assignment')
            ->select('driver_assignment.*, vehicle.vehicle_no as truck_no, vehicle.id as truck_id')
            ->join('vehicle', 'vehicle.id = driver_assignment.vehicle_no')
        ...
        if ($assignment) {
            return $this->response->setJSON(['status' => 'success', 'vehicle_no' => $assignment->truck_no, 'vehicle_id' => $assignment->truck_id]);
        } else {
            return $this->response->setJSON(['status' => 'error', 'message' => 'No vehicle assigned']);
        }
    }
    ...
    public function edit_extra_diesel()
    {
        ...
        echo '      </select>
                ...
                <div class="form-group">
                    <label>Driver</label>
                    <select class="form-control" id="driver_select_edit" disabled required>';
        foreach ($data['drivers'] as $dr) {
            $sel = ($dr->id == $data['row']->driver_id) ? 'selected' : '';
            echo '<option value="' . $dr->id . '" ' . $sel . '>' . $dr->name . '</option>';
        }
        echo '      </select>
                    <input type="hidden" name="driver" id="driver_hidden_edit" value="' . $data['row']->driver_id . '">
                </div>
                <div class="form-group">
                    <label>Date</label>
    ...
    public function edit_passenger_diesel()
    {
        ...
        echo '<form action="' . base_url('Admin/update_passenger_diesel') . '" method="post">
                ...
                <div class="form-group">
                    <label>Vehicle</label>
                    <select class="form-control select2-search" name="vehicle" required>';
        foreach ($data['vehicle'] as $v) {
            $sel = ($v->id == $data['row']->vehicle_id) ? 'selected' : '';
            echo '<option value="' . $v->id . '" ' . $sel . '>' . $v->vehicle_no . '</option>';
        }
        echo '      </select>
                ...
                <div class="form-group">
                    <label>Location</label>
                    <select class="form-control select2-search" name="location" required>';
        ...
        echo '      </select>
                ...
                <div class="form-group">
                    <label>Issued By</label>
                    <select class="form-control select2-search" name="issued_by" required>';
        foreach ($data['issuers'] as $u) {
            $sel = ($u->id == $data['row']->issued_by) ? 'selected' : '';
            echo '<option value="' . $u->id . '" ' . $sel . '>' . $u->full_name . '</option>';
        }
    ...
    public function excel_dieselentry()
    {
        $file = $this->request->getFile('file');
        ...
        if ($file->isValid() && !$file->hasMoved()) {
            ...
            for ($rowIndex = 2; $rowIndex <= $highestRow; $rowIndex++) {
                ...
                $vendor_code = trim((string) $sheet->getCell('A' . $rowIndex)->getValue());
                $vehicle_no = trim((string) $sheet->getCell('B' . $rowIndex)->getValue());

                // Skip truly empty rows
                if (empty($vendor_code) && empty($vehicle_no)) {
                    continue;
                }

                $cell = $sheet->getCell('C' . $rowIndex);
                $des_date = $cell->getValue();

                if (empty($des_date)) {
                    $des_date = $cell->getFormattedValue(); // fallback
                }
                ...
                if (!empty($des_date)) {
                    try {
                        if (is_numeric($des_date)) {
                            $date = \PhpOffice\PhpSpreadsheet\Shared\Date::excelToDateTimeObject($des_date)->format('Y-m-d');
                        } else {
                            $des_date = str_replace('/', '-', trim($des_date));
                            $d = DateTime::createFromFormat('d-m-Y', $des_date);
                            if ($d) {
                                $date = $d->format('Y-m-d');
                            } else {
                                $timestamp = strtotime($des_date);
                ...
                $qty = (float) $sheet->getCell('D' . $rowIndex)->getValue();
                $rate = (float) $sheet->getCell('E' . $rowIndex)->getValue();

                // Validation with break
                if (empty($vendor_code)) {
                    $errorMsg = "Row $rowIndex: Vendor name is missing.";
                    break;
                }
                if (empty($vehicle_no)) {
                    $errorMsg = "Row $rowIndex: Vehicle number is missing.";
                    break;
                }
                if ($qty <= 0) {
                    $errorMsg = "Row $rowIndex: Invalid Quantity ($qty).";
                    break;
                }
                if ($rate <= 0) {
                    $errorMsg = "Row $rowIndex: Invalid Rate ($rate).";
                    break;
                }

                // Vendor lookup
                $vendor = $this->db->table('vendor')
                    ->select('id')
                    ->where('name', $vendor_code)
                    ->get()
                    ->getRow();

                if (!$vendor) {
                    $errorMsg = "Row $rowIndex: Vendor '$vendor_code' not found in database.";
                    break;
                }
                $vendor_id = $vendor->id;
                ...
                if (!$vehicle) {
                    $errorMsg = "Row $rowIndex: Vehicle '$vehicle_no' not found in database.";
                    break;
                }
                ...
                try {
                    ...
                    $errorMsg = "Row $rowIndex: Database error - " . $e->getMessage();
                    break;
                }
            }

            if (!empty($errorMsg)) {
                return redirect()->to(base_url('/Admin/diesel_entry'))
                    ->with('error', $successCount . " records uploaded successfully. Stopped at " . $errorMsg);
            }

            return redirect()->to(base_url('/Admin/diesel_entry'))
                ->with('success', $successCount . " Diesel data uploaded successfully.");
        }
    ...
    function edit_doregistration()
    {
        ...
        <form action="<?= base_url(); ?>/Admin/update_do_registration" enctype="multipart/form-data" method="post">
            <div class="uk-margin-bottom">
                <label>DO No</label>
                <input type="hidden" name="doreg_no" id="edit_doreg_no" class="form-control"
                    value="<?= $doreg->do_registration_id ?>" readonly />
                <input type="text" name="do_no" id="edit_do_no" class="form-control" value="<?= $doreg->do_no ?>" />
                <?php if (isset($validation)) { ?><span
                        class="text-danger"><?= $validation->getError('do_no'); ?></span><?php } ?>
            </div>
            <div class="uk-margin-bottom">
                <label>Route</label>
                <select class="js-states form-control" name="route" id="edit_route">
                    ...
                    <?php foreach ($route as $rut) { ?>
                        <option value="<?= $rut->id ?>" <?= $doreg->route_id == $rut->id ? 'selected' : '' ?>>
                            (<?= $rut->location_shortname ?>) <?= $rut->from_city ?> === <?= $rut->to_city ?></option>
                    <?php } ?>
                </select>
                <?php if (isset($validation)) { ?><span
                        class="text-danger"><?= $validation->getError('route'); ?></span><?php } ?>
            </div>
            ...
            <div class="uk-margin-bottom">
                ...
                <input type="number" class="form-control uk-margin-small-top" name="diesel" value="<?= $diesel_input_val ?>"
                    min='0' />
                <?php if (isset($validation)) { ?><span
                        class="text-danger"><?= $validation->getError('diesel'); ?></span><?php } ?>
            </div>
            ...
            <div class="uk-child-width-1-2@m uk-grid-small edit_trip_expenses_field" uk-grid>
                <div class="uk-margin-bottom">
                    ...
                    <input type="number" name="1trip_expenses" id="1trip_expenses" class="form-control"
                        value="<?= $doreg->trip_expenses1 ?>" min='0' />
                    <?php if (isset($validation)) { ?><span
                            class="text-danger"><?= $validation->getError('1trip_expenses'); ?></span><?php } ?>
                </div>
                <div class="uk-margin-bottom">
                    <label>2Trip Expenses</label>
                    <input type="number" name="2trip_expenses" id="2trip_expenses" class="form-control"
                        value="<?= $doreg->trip_expenses2 ?>" min='0' />
                    <?php if (isset($validation)) { ?><span
                            class="text-danger"><?= $validation->getError('2trip_expenses'); ?></span><?php } ?>
                </div>
                <div class="uk-margin-bottom">
                    <label>3Trip Expenses</label>
                    <input type="number" name="3trip_expenses" id="3trip_expenses" class="form-control"
                        value="<?= $doreg->trip_expenses3 ?>" min='0' />
                    <?php if (isset($validation)) { ?><span
                            class="text-danger"><?= $validation->getError('3trip_expenses'); ?></span><?php } ?>
                </div>
                <div class="uk-margin-bottom">
                    <label>4Trip Expenses</label>
                    <input type="number" name="4trip_expenses" id="4trip_expenses" class="form-control"
                        value="<?= $doreg->trip_expenses4 ?>" min='0' />
                    <?php if (isset($validation)) { ?><span
                            class="text-danger"><?= $validation->getError('4trip_expenses'); ?></span><?php } ?>
                </div>
                <div class="uk-margin-bottom">
                    <label>5Trip Expenses</label>
                    <input type="number" name="5trip_expenses" id="5trip_expenses" class="form-control"
                        value="<?= $doreg->trip_expenses5 ?>" min='0' />
                    <?php if (isset($validation)) { ?><span
                            class="text-danger"><?= $validation->getError('5trip_expenses'); ?></span><?php } ?>
                </div>
                <div class="uk-margin-bottom">
                    <label>6Trip Expenses</label>
                    <input type="number" name="6trip_expenses" id="6trip_expenses" class="form-control"
                        value="<?= $doreg->trip_expenses6 ?>" min='0' />
                    <?php if (isset($validation)) { ?><span
                            class="text-danger"><?= $validation->getError('6trip_expenses'); ?></span><?php } ?>
                </div>
            </div>
            ...
            <div class="uk-margin-bottom">
                ...
                <input type="number" step="0.01" name="diesel_rate" id="edit_diesel_rate" class="form-control"
                    value="<?= $doreg->diesel_rate ?>" />
            </div>
            ...
            <div class="uk-margin-bottom">
                ...
                <?php if (isset($validation)) { ?><span
                        class="text-danger"><?= $validation->getError('from_date'); ?></span><?php } ?>
            </div>
            <div class="uk-margin-bottom">
                <label>To Date</label>
                <input type="date" name="to_date" id="edit_to_date" class="form-control" value="<?= $doreg->to_date ?>" />
                <?php if (isset($validation)) { ?><span
                        class="text-danger"><?= $validation->getError('to_date'); ?></span><?php } ?>
            </div>
            <div class="uk-margin-bottom">
                <label>Party</label>
                ...
                <?php if (isset($validation)) { ?><span
                        class="text-danger"><?= $validation->getError('party'); ?></span><?php } ?>
            </div>
            <div class="uk-margin-bottom">
                <label>Rate</label>
                <input type="number" class="form-control" name="rate" id="edit_rate" value="<?= $doreg->rate ?>" min='0' />
                <?php if (isset($validation)) { ?><span
                        class="text-danger"><?= $validation->getError('rate'); ?></span><?php } ?>
            </div>

            <div class="uk-margin-bottom">
                <label>TDS Rate (%)</label>
                <input type="number" step="0.01" name="tds_percentage" id="edit_tds_percentage" class="form-control"
                    value="<?= $doreg->tds_percentage ?? 2.00; ?>" />
            </div>
            <div class="uk-margin-bottom">
                <label>Shortage Qty</label>
                <input type="number" step="0.01" name="shortage_qty" id="edit_shortage_qty" class="form-control"
                    value="<?= $doreg->shortage_qty ?>" />
            </div>
            <div class="uk-margin-bottom">
                <label>Shortage Rate</label>
                <input type="number" step="0.01" name="shortage_rate" id="edit_shortage_rate" class="form-control"
                    value="<?= $doreg->shortage_rate ?>" />
            </div>
            <div class="uk-margin-bottom" style="display: flex; align-items: center;">
                <input type="checkbox" name="special_shortage" id="edit_special_shortage" class="uk-checkbox" value="1"
                    <?= ($doreg->special_shortage == 1) ? 'checked' : '' ?> />
    ...
    function changeprice_doregistration()
    {
        ...
        <form action="<?= base_url(); ?>/Admin/changeprice_diesel_registration" method="post">
            <div class="uk-child-width-1-2@m uk-grid-small uk-grid" uk-grid="">
                ...
                <div class="uk-margin-bottom uk-first-column">
                    <label>Diesel Rate (₹)</label>
                    <input type="number" step="0.01" name="diesel_rate" placeholder="Enter Diesel Rate" class="form-control"
                        value="" required>
                </div>
            </div>
        ...
        <form action="<?= base_url(); ?>/Admin/insert_shortage" method="post">
            <div class="uk-child-width-1-1@m uk-grid-small uk-grid" uk-grid>
                ...
                <div class="uk-margin-bottom">
                    ...
                    <div>
                        <label>
                            <input type="radio" name="special_deduction" value="1" <?= isset($do_pric->special_deduction) && $do_pric->special_deduction == 1 ? 'checked' : '' ?>> Yes
                        </label>
                        <label>
                            <input type="radio" name="special_deduction" value="0" <?= !isset($do_pric->special_deduction) || $do_pric->special_deduction == 0 ? 'checked' : '' ?>> No
                        </label>
                    </div>
                </div>
    ...
    public function Collection()
    {
        ...
        $data['total_count'] = $this->AdminModel->despatch_count($from_date, $to_date, $do_no, $chalan_status, $payment_status, $deposited_status, $voucher_id, $search);
        $data['despatch'] = $this->AdminModel->despatch_dtls1_paginated($from_date, $to_date, $do_no, $chalan_status, $payment_status, $deposited_status, $records_per_page, $offset, $voucher_id, $search);

        $data['date'] = ['from_date' => $from_date, 'to_date' => $to_date, 'do_no' => $do_no, 'voucher_id' => $voucher_id, 'chalan_status' => $chalan_status, 'payment_status' => $payment_status, 'deposited_status' => $deposited_status, 'search' => $search];
        $data['current_page'] = $current_page;
        $data['records_per_page'] = $records_per_page;
    ...
    function edit_despatchd()
    {
        ...
        <form action="<?php echo base_url(); ?>/Admin/edit_despatch_entry" enctype="multipart/form-data" method="post">
            <div class="uk-margin-bottom">
                ...
                <select class="form-control select2-edit" name="vehicle_no" required>
            ...
            <div class="uk-margin-bottom">
                ...
                <input type="text" name="quantity" placeholder="enter Quantity" id="quantity" class="form-control"
                    value="<?= $des->quantity; ?>" required />
                <?php if (isset($validation)) { ?><span
                        class="text-danger"><?= $error = $validation->getError('quantity'); ?></span><?php } ?>
            </div>
            <div class="uk-margin-bottom">
                <lable>DO No</lable>
                <select class="form-control select2-edit" name="do_no" required>
                ...
                <?php if (isset($validation)) { ?><span
                        class="text-danger"><?= $error = $validation->getError('do_no'); ?></span><?php } ?>
            </div>
            <div class="uk-margin-bottom">
                <lable>Ref No.</lable>
                <input type="text" name="ref_no" placeholder="enter Ref No.  " id="ref_no" class="form-control"
                    value="<?= $des->ref_no; ?>" required />
                <?php if (isset($validation)) { ?><span
                        class="text-danger"><?= $error = $validation->getError('ref_no'); ?></span><?php } ?>
            </div>
    ...
    function Subadmin()
    {
        if ($this->session->get('user_id')) {
            ...
            $data['setting'] = $this->AdminModel->Settingdata();
            $data['singleuser'] = $this->AdminModel->userdata($user_id);
            $data['allsubadmin'] = $this->AdminModel->asObject()->where('user_type', 2)->where('deleted_by', null)->paginate(10);
    ...
    function billing_details()
    {
        ...
        <table width="100%" border="0" cellpadding="5" cellspacing="0" style="border:solid 1px #ccc;"
    ...
    public function openinghsddtl()
    {
        ...
        <div class="form-group">
            ...
            <input type="text" value="<?= htmlspecialchars($closing_hsd); ?>" class="form-control" name="opening_hsd"
                id="opening_hsd" required>
        </div>

        <div class="form-group">
            <label for="opening_km">Opening KM:</label>
            <input type="text" value="<?= htmlspecialchars($closing_km); ?>" class="form-control" name="opening_km"
                id="opening_km" required>
        </div>
        <?php
    }
    ...
    public function scrapTyer_management()
    {
        if ($this->session->get('user_id') == '') {
            return redirect()->to(base_url('login'));
        }
        ...
        $data['setting'] = $this->AdminModel->Settingdata();
        $data['singleuser'] = $this->AdminModel->userdata($user_id);
        $data['vendors'] = $this->AdminModel->Get_vendor();
        $data['tyer_list'] = $this->db->table('tyer_management tm')
            ->select('tm.*,l.location_name')
            ->join('location l', 'l.location_id = tm.location_id', 'left')
            ->where('tm.status', 3)
    ...
    public function scrapTyreBackToStock($id = null)
    {
        if ($this->session->get('user_id') == '') {
            return redirect()->to(base_url('login'));
        }
        ...
        return redirect()->to('Admin/scrapTyer_management');
    }

    public function sent_to_vendor()
    {
        if (strtolower($this->request->getMethod()) !== 'post') {
            return $this->response->setJSON(['status' => 'error', 'message' => 'Invalid request method.']);
        }

        $id = $this->request->getPost('tyer_id');
        $remark = $this->request->getPost('remark');

        $this->db->transStart();

        // 1. Update Status to 10 (Exchange Requested)
        $this->db->table('tyer_management')->where('id', $id)->update([
            'status' => 10,
            'remark' => "EXCHANGE REQUESTED: $remark"
        ]);

        // 2. Log History
        $this->db->table('tyer_management_history')->insert([
            'tyre_id' => $id,
            'event_type' => 10, // Exchange Requested
            'event_date' => date('Y-m-d'),
            'remarks' => "EXCHANGE INITIATED: $remark",
            'created_at' => date('Y-m-d H:i:s')
        ]);
        ...
        return $this->response->setJSON(['status' => 'success', 'message' => 'Exchange requested successfully.']);
    }

    public function sentToVendorTyer_management()
    {
        if ($this->session->get('user_id') == '') {
            return redirect()->to(base_url('login'));
        }
        ...
        $data['tyer_list'] = $this->db->table('tyer_management tm')
            ...
            ->where('tm.status', 10) // Exchange Requested
            ->get()
            ->getResult();

        return view('admin/sentToVendorTyer_management_vw', $data);
    }

    public function soldTyer_management()
    {
        if ($this->session->get('user_id') == '') {
            return redirect()->to(base_url('login'));
        }
        ...
        $data['tyer_list'] = $this->db->table('tyer_management tm')
            ...
            ->join('location l', 'l.location_id = tm.location_id', 'left')
            ->join('vendor v', 'v.id = tm.vendor_id', 'left')
            ->where('tm.status', 7) // Sold
            ->get()
            ->getResult();

        return view('admin/soldTyer_management_vw', $data);
    }

    public function process_tyre_sale()
    {
        if ($this->session->get('user_id') == '') {
            return redirect()->to(base_url('login'));
        }
    ...
    public function tyre_exchange_report()
    {
        ...
        $data['setting'] = $this->AdminModel->Settingdata();
        $data['singleuser'] = $this->AdminModel->userdata($user_id);
        $data['history'] = $this->AdminModel->getExchangeHistory($filters);
        $data['vehicles'] = $this->AdminModel->Getallvehicle();

        return view('admin/tyre_exchange_report_vw', $data);
    }

    public function vendor_exchange($id = null)
    {
        if ($this->session->get('user_id') == '') {
            return redirect()->to(base_url('login'));
        }

        $data['tyer'] = $this->db->table('tyer_management tm')
            ->select('tm.*, v.name as vendor_name')
            ->join('vendor v', 'v.id = tm.vendor_id', 'left')
            ->where('tm.id', $id)
            ->get()
            ->getRow();
        ...
        $data['brands'] = $this->db->table('tyer_management')->select('brand_name')->distinct()->get()->getResult();
        $user_id = $this->session->get('user_id');
        $data['setting'] = $this->AdminModel->Settingdata();
        $data['singleuser'] = $this->AdminModel->userdata($user_id);

        return view('admin/vendor_exchange_vw', $data);
    }

    public function process_vendor_exchange()
    {
        if ($this->session->get('user_id') == '') {
            return redirect()->to(base_url('login'));
        }
    ...
    public function insert_tyer()
    {
        ...
        if (empty($tyer_sl_nos)) {
            return redirect()->back()->withInput()->with('error', 'Please add at least one tyre.');
        }
        ...
        if ($existingBill) {
            $this->db->transRollback();
            return redirect()->back()->withInput()->with('error', 'Bill number already exists: ' . $bill_no);
        }

        // Check for duplicate serial numbers in the current list and in the database
        $temp_sl_nos = [];
        foreach ($tyer_sl_nos as $sl_no) {
            $sl_no = trim($sl_no);
            if (empty($sl_no))
                continue;

            if (in_array($sl_no, $temp_sl_nos)) {
                $this->db->transRollback();
                return redirect()->back()->withInput()->with('error', 'Duplicate Serial Number in your list: ' . $sl_no);
            }
            $temp_sl_nos[] = $sl_no;

            $existingSerial = $this->db->table('tyer_management')
                ->where('tyer_sl_no', $sl_no)
                ->get()
                ->getRow();
            ...
            if ($existingSerial) {
                $this->db->transRollback();
                return redirect()->back()->withInput()->with('error', 'Tyre Serial Number already exists in database: ' . $sl_no);
            }
        }
        ...
        // Complete Transaction to get IDs
        $this->db->transComplete();

        if ($this->db->transStatus() === FALSE) {
            return redirect()->back()->withInput()->with('error', 'Failed to save tyre data. Please check your input.');
        }

        // After successful transaction, add to history
        // Targeted fetch using bill_no and date to avoid grabbing unrelated records
        $tyres = $this->db->table('tyer_management')
            ->where('bill_no', $bill_no)
            ->where('date', $date)
            ->get()
        ...
        foreach ($tyres as $tyre) {
            // Check if history already exists to avoid duplicates if re-running
            $exists = $this->db->table('tyer_management_history')
                ->where(['tyre_id' => $tyre->id, 'event_type' => 1])
                ->countAllResults();

            if ($exists == 0) {
                $data1 = [
                    'tyre_id' => $tyre->id,
                    'event_type' => 1, // Purchase / In-Stock
                    'location_id' => $location_id,
                    'event_date' => $date,
                    'vendor_id' => $vendor_id,
                ];
        ...
        return redirect()->to('admin/tyer_management')->with('success', 'Tyre bill added successfully.');
    }
    ...
    function gettyer()
    {
        ...
        <select class="form-control form-control-modern" id="quickTyreSelect" name="tyer_id">
    ...
    public function exchange_tyer_data()
    {
        ...
        // Find existing tyre at this position
        $old_tyre = $this->db->table('tyer_management')
            ->where('vehicle_id', $vehicle_id)
            ->where('tyer_position', $tyer_position)
            ->get()->getRow();

        if ($old_tyre) {
            // Move old tyre to stock
            ...
            // Log history for old tyre removal
            $this->db->table('tyer_management_history')->insert([
                'tyre_id' => $old_tyre->id,
                'event_type' => 4, // Exchange/Remove
                'location_id' => $location,
        ...
        // Log history for new tyre assignment
        $this->db->table('tyer_management_history')->insert([
            'tyre_id' => $tyer_id,
            'event_type' => 3, // Assign
            'event_date' => $replacement_date,
    ...
    public function backToStock_tyer_data()
    {
        $vehicle_id = $this->request->getVar('vehicle_id');
        $tyer_position = $this->request->getVar('tyer_position');
        $location = $this->request->getVar('location');
        ...
        if ($old_tyre) {
            ...
            return redirect()->to('admin/Asign_Tyer')->with('msg', 'Tyre moved back to stock successfully.');
        }
    ...
    public function tyer_report()
    {
        ...
        if ($status === null && $this->request->getMethod() !== 'post') {
            $status = 2; // Default to Assigned
        }
        ...
        if ($status != '') {
            $builder->where('tm.status', $status);
        }
        $builder->where('tm.status !=', 1);
    ...
    {
        $type = strtoupper($this->request->getGet('type') ?? 'STAFF');

        $spreadsheet = new \PhpOffice\PhpSpreadsheet\Spreadsheet();
        $sheet = $spreadsheet->getActiveSheet();

        // Conditional required fields
        $dlNumberLabel = ($type === 'DRIVER') ? 'DL Number (For Driver)*' : 'DL Number';
        $dlExpiryLabel = ($type === 'DRIVER') ? 'DL Expiry (For Driver)*' : 'DL Expiry';
        ...
        foreach ($headers as $header) {
            ...
            // If field is required (contains *), mark it Red
            if (strpos($header, '*') !== false) {
                $sheet->getStyle($col . '1')->getFont()->getColor()->setARGB(\PhpOffice\PhpSpreadsheet\Style\Color::COLOR_RED);
            }
        ...
        // Initialize counters for examples
        $rowNum = 2;
        $serialNo = 1;

        // Example for Driver
        if ($type === 'DRIVER') {
            $sheet->setCellValue('A' . $rowNum, $serialNo++);
            $sheet->setCellValue('B' . $rowNum, 'DRIVER');
            $sheet->setCellValue('C' . $rowNum, 'Example Driver');
            $sheet->setCellValue('D' . $rowNum, date('d/m/Y'));
            $sheet->setCellValue('E' . $rowNum, '15000');
            $sheet->setCellValue('F' . $rowNum, 'SBI');
            $sheet->setCellValue('G' . $rowNum, '1234567890');
            $sheet->setCellValue('H' . $rowNum, 'SBIN0001234');
            $sheet->setCellValue('I' . $rowNum, 'DL-123456789');
            $sheet->setCellValue('J' . $rowNum, date('d/m/Y', strtotime('+5 years')));
            $sheet->setCellValue('K' . $rowNum, '123456789012');
            $sheet->setCellValue('L' . $rowNum, 'ABCDE1234F');
            $sheet->setCellValue('M' . $rowNum, '9876543210');
            $sheet->setCellValue('N' . $rowNum, 'Father Name');
            $sheet->setCellValue('O' . $rowNum, 'Spouse Name');
            $sheet->setCellValue('P' . $rowNum, '01/01/1990');
            $sheet->setCellValue('Q' . $rowNum, '9123456789');
            $sheet->setCellValue('R' . $rowNum, 'B+');
            $sheet->setCellValue('S' . $rowNum, '0');
            $sheet->setCellValue('T' . $rowNum, 'Main Office');
            $sheet->setCellValue('U' . $rowNum, 'Address Detail');
            $rowNum++;
        }
        ...
        if ($type === 'STAFF') {
            $sheet->setCellValue('A' . $rowNum, $serialNo++);
            $sheet->setCellValue('B' . $rowNum, 'STAFF');
            $sheet->setCellValue('C' . $rowNum, 'Example Staff');
            $sheet->setCellValue('D' . $rowNum, date('d/m/Y'));
            $sheet->setCellValue('E' . $rowNum, '20000');
            $sheet->setCellValue('F' . $rowNum, 'HDFC');
            $sheet->setCellValue('G' . $rowNum, '0987654321');
            $sheet->setCellValue('H' . $rowNum, 'HDFC0004321');
            $sheet->setCellValue('K' . $rowNum, '234567890123');
            $sheet->setCellValue('L' . $rowNum, '');
            $sheet->setCellValue('M' . $rowNum, '8765432109');
            $sheet->setCellValue('T' . $rowNum, 'Main Office');
            $rowNum++;
        }
        ...
        if ($type === 'MECHANIC') {
            $sheet->setCellValue('A' . $rowNum, $serialNo++);
            $sheet->setCellValue('B' . $rowNum, 'MECHANIC');
            $sheet->setCellValue('C' . $rowNum, 'Example Mechanic');
            $sheet->setCellValue('D' . $rowNum, date('d/m/Y'));
            $sheet->setCellValue('E' . $rowNum, '18000');
            $sheet->setCellValue('K' . $rowNum, '345678901234');
            $sheet->setCellValue('L' . $rowNum, '');
            $sheet->setCellValue('M' . $rowNum, '7654321098');
            $rowNum++;
        }

        $writer = new \PhpOffice\PhpSpreadsheet\Writer\Xlsx($spreadsheet);
        $filename = 'staff_import_sample_' . strtolower($type) . '.xlsx';
    ...
    function expert_excel()
    {
        ...
        $tyer_data = $builder->get()->getResult();
        ...
        // Status mapping for human readable output
        $statusMap = [
            1 => 'In Stock',
            2 => 'Assigned',
            3 => 'Scrap Yard',
        ...
        foreach ($tyer_data as $tyer) {
            $statusText = isset($statusMap[$tyer->status]) ? $statusMap[$tyer->status] : 'Unknown';

            $sheet->setCellValue('A' . $rowNumber, $slNo++);
            $sheet->setCellValue('B' . $rowNumber, $tyer->tyer_sl_no);
            $sheet->setCellValue('C' . $rowNumber, $tyer->brand_name . ' (' . $tyer->tyer_type . ' | ' . $tyer->model . ')');
            $sheet->setCellValue('D' . $rowNumber, $tyer->bill_no);
            $sheet->setCellValue('E' . $rowNumber, $tyer->location_name);
            $sheet->setCellValue('F' . $rowNumber, $tyer->tyre_condition);
        ...
        $filename = 'tyre_report_export_' . date('Ymd_His') . '.xlsx';
...
";
    ...
    public function getParticularsByGroup()
    {
        ...
        if ($group_id == 6) { // Vendor
            ...
            foreach ($res as $r)
                $data[] = ['id' => $r->id, 'name' => $r->vehicle_no];
        } elseif ($group_id == 2) { // Cash Book
            $res = $this->db->query("SELECT * FROM location WHERE (status IS NULL OR status='Active')")->getResult();
            foreach ($res as $r)
                $data[] = ['id' => $r->location_id, 'name' => $r->location_name];
        } elseif ($group_id == 7) { // Bank
            $res = $this->db->table('bank')->get()->getResult();
            foreach ($res as $r)
                $data[] = ['id' => $r->id, 'name' => $r->bank_name];

            // Also include ledgers in the Bank group if any
            $ledgers = $this->db->table('ledger')->where('group_id', $group_id)->get()->getResult();
            foreach ($ledgers as $l)
                $data[] = ['id' => $l->ledger_id, 'name' => $l->ledger_name];
        } else {
            $res = $this->db->table('ledger')->where('group_id', $group_id)->get()->getResult();
            foreach ($res as $r)
                $data[] = ['id' => $r->ledger_id, 'name' => $r->ledger_name];
        }
    ...
    public function saveVoucher()
    {
        ...
        if ($this->AdminModel->saveVoucher($voucherData, $entries)) {
            ...
            $last_err = session()->getFlashdata('last_db_error');
            $db_error = $this->db->error();
            $error_msg = 'Failed to save voucher. ';
            
            if ($last_err) {
                $error_msg .= 'Database Error: ' . $last_err;
            } elseif (!empty($db_error['message'])) {
                $error_msg .= 'Database Error: ' . $db_error['message'];
            } else {
                $error_msg .= 'Unknown database error occurred.';
            }
            return redirect()->back()->withInput()->with('error', $error_msg);
        }
    }
    ...
    public function ledger_statement()
    {
        ...
        if ($group_id) {
            ...
            if ($group_id == 6) { // Vendor
                $res = $this->AdminModel->Get_vendor();
                foreach ($res as $r)
                    $data['particulars'][] = ['id' => $r->id, 'name' => $r->name];
            } elseif ($group_id == 5) { // Staff
                $res = $this->db->table('staff')->where('user_type', 'STAFF')->get()->getResult();
                foreach ($res as $r)
                    $data['particulars'][] = ['id' => $r->id, 'name' => $r->name];
                ...
                $res = $this->AdminModel->Getvehicle();
                foreach ($res as $r)
                    $data['particulars'][] = ['id' => $r->id, 'name' => $r->vehicle_no];
            } elseif ($group_id == 2) { // Cash Book
                $res = $this->db->query("SELECT * FROM location WHERE (status IS NULL OR status='Active')")->getResult();
                foreach ($res as $r)
                    $data['particulars'][] = ['id' => $r->location_id, 'name' => $r->location_name];
            } elseif ($group_id == 7) { // Bank
                $res = $this->db->table('bank')->get()->getResult();
                foreach ($res as $r)
                    $data['particulars'][] = ['id' => $r->id, 'name' => $r->bank_name];
            } else {
                $res = $this->db->table('ledger')->where('group_id', $group_id)->get()->getResult();
                foreach ($res as $r)
                    $data['particulars'][] = ['id' => $r->ledger_id, 'name' => $r->ledger_name];
            }
        }

        // Fetch statement based on provided filters (or default dates if none)
    ...
    public function download_staffadvance_sample()
    {
        $spreadsheet = new Spreadsheet();
        $sheet = $spreadsheet->getActiveSheet();
        ...
        $writer = new XlsxWriter($spreadsheet);
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment; filename="staff_advance_sample.xlsx"');
        header('Cache-Control: max-age=0');
        $writer->save('php://output');
        exit;
    }

    public function download_diesel_sample()
    {
        $spreadsheet = new Spreadsheet();
        $sheet = $spreadsheet->getActiveSheet();
        ...
        // Sample data
        $sheet->setCellValue('A2', 'Pump A');
        $sheet->setCellValue('B2', 'MH01AB1234');
        $sheet->setCellValue('C2', date('d/m/Y'));
        $sheet->setCellValue('D2', '50.5');
        $sheet->setCellValue('E2', '95.5');

        $writer = new XlsxWriter($spreadsheet);
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment; filename="diesel_entry_sample.xlsx"');
        header('Cache-Control: max-age=0');
        $writer->save('php://output');
        exit;
    }

    public function download_despatch_sample()
    {
        $spreadsheet = new Spreadsheet();
        $sheet = $spreadsheet->getActiveSheet();
        $sheet->setCellValue('A1', 'Date (dd/mm/yyyy)');
        $sheet->setCellValue('B1', 'Vehicle No');
        $sheet->setCellValue('C1', 'DO No');
        $sheet->setCellValue('D1', 'Quantity');
        ...
        $sheet->setCellValue('B2', 'MH01AB1234');
        $sheet->setCellValue('C2', 'DO-1001');
        $sheet->setCellValue('D2', '20');
        $sheet->setCellValue('E2', 'REF-555');

        $writer = new XlsxWriter($spreadsheet);
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment; filename="despatch_entry_sample.xlsx"');
        header('Cache-Control: max-age=0');
        $writer->save('php://output');
        exit;
    }
    ...
    public function vehicle_types()
    {
        ...
        return view('admin/vehicle_types_vw', $data);
    }
    ...
    public function insert_vehicle_type()
    {
        ...
        $model->insert($data);
        return redirect()->to('Admin/vehicle_types')->with('msg', 'Vehicle Type added successfully');
    }
    ...
    public function edit_vehicle_type()
    {
        ...
        if ($type) {
            ...
            echo json_encode(['status' => 'error', 'message' => 'Type not found']);
        }
    }
    ...
    public function update_vehicle_type()
    {
        ...
        return redirect()->to('Admin/vehicle_types')->with('msg', 'Vehicle Type updated successfully');
    }
    ...
    public function delete_vehicle_type($id)
    {
        $model = new VehicleTypeModel();
        $model->delete($id);
        return redirect()->to('Admin/vehicle_types')->with('msg', 'Vehicle Type deleted successfully');
    }
}